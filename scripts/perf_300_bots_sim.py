...
async def main():
    scheduler = BotScheduler(db_sem_limit=20, compute_sem_limit=50, binance_sem_limit=10)
    now = time.monotonic()
    ...
    try:
        ...
        pass
    if depths:
        print(f"queue_depth min={min(depths)} max={max(depths)} avg={sum(depths)/len(depths):.1f} samples={len(depths)}")
