...
def main():
    if not _MANAGER_PY.exists():
        print("fix_cgi_once: manager_backend.py bulunamadi.", file=sys.stderr)
        return 1
    text = _MANAGER_PY.read_text(encoding="utf-8", errors="replace")
    # Zaten guncel (dual-version veya patched) ise dokunma
    if "def _parse_multipart_form" in text and "import cgi" not in text:
        return 0
    if "cgi = None" in text and "except ModuleNotFoundError" in text:
        return 0
    ...
    if changed:
        ...
        print("fix_cgi_once: manager_backend.py Python 3.13+ icin duzeltildi.")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("fix_cgi_once HATA: %s" % e, file=sys.stderr)
        sys.exit(1)
