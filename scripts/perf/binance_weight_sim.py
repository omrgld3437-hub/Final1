...
def main():
    ...
    ap.add_argument(
        "--poll", type=float, default=3.0, help="Snapshot poll interval (seconds)"
    )
    ap.add_argument(
        "--trades", type=float, default=2.0, help="Worker trades per minute"
    )
    ap.add_argument(
        "--datahub", type=int, default=12, help="DataHub weight/min (ticker/price 2*6)"
    )
    args = ap.parse_args()
    ...
    safe_users = (
        int(
            (BINANCE_WEIGHT_LIMIT - order_weight_per_min - datahub_weight)
            / ((60 / poll) * WEIGHT_ACCOUNT)
        )
        if poll > 0
        else 0
    )
    ...
    print(
        f"Account weight/min: {account_weight_per_min:.0f} ({account_calls_per_min:.0f} calls)"
    )
