...
def test_build_client_order_id_format():
    ...
    coid = build_client_order_id(1, 1, "BTCUSDT", "BUY", 0.001, 50.0, "trail_buy_grid", 0)
    assert len(coid) <= 36
...
def test_build_client_order_id_same_intent_same_hash_part():
    ...
    coid1 = build_client_order_id(1, 1, "BTCUSDT", "BUY", 0.001, 50.0, "trail_buy_grid", 0, epoch_ms=1000)
    coid2 = build_client_order_id(1, 1, "BTCUSDT", "BUY", 0.001, 50.0, "trail_buy_grid", 0, epoch_ms=1000)
    assert coid1 == coid2


def test_build_intent_id_includes_run_id():
    """Different run_id => different intent_id (no restart collision)."""
    i0 = build_intent_id(1, 0, "LTCUSDT", "BUY", 0.1, 1400.0, "initial_allocation", None, run_id="cmd101")
    i1 = build_intent_id(1, 0, "LTCUSDT", "BUY", 0.1, 1400.0, "initial_allocation", None, run_id="cmd102")
    assert i0 != i1
...
def test_build_client_order_id_includes_run_id_max_36():
    ...
    coid = build_client_order_id(1, 0, "LTCUSDT", "BUY", 0.1, 1400.0, "initial_allocation", None, run_id="cmd999")
    assert len(coid) <= 36
