...
def test_all_grids_above_min_passes():
    cfg = _cfg(
        100,
        sell_grids=[{"sell_qty_pct_of_base": 50}],
        buy_grids=[{"buy_qty_pct_of_quote": 50}],
    )
    ok, msg, viol, min_b = validate_dca_grid_notionals(cfg)
    assert ok
    assert not viol
...
def test_compute_min_budget_20_usd_40pct_sell():
    cfg = _cfg(
        20,
        sell_grids=[{"sell_qty_pct_of_base": 40}],
        buy_grids=[{"buy_qty_pct_of_quote": 40}],
    )
    ...
    ok, msg, viol, min_b = validate_dca_payload(
        {
            ...
            "up": {
                "grids": [
                    {"trigger_pct": 1, "qty_pct": 40},
                    {"trigger_pct": 2, "qty_pct": 60},
                ]
            },
