...
def test_paper_buy_deducts_base_fee():
    net, quote, fill_px, fee_usdt = paper_buy_from_quote(100.0, 2000.0, symbol="ETHUSDT")
    ...
    _, asset, fee = parse_fill_commission(fill["fills"], "ETHUSDT", float(fill["fills"][0]["price"]))
    assert asset == "ETH"
    assert fee == pytest.approx(fee_usdt, rel=1e-4)


def test_paper_sell_deducts_usdt_fee():
    sold, net_quote, fill_px, fee_usdt = paper_sell_from_base(0.5, 2000.0, symbol="ETHUSDT")
