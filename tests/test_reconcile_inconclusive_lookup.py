...
def test_network_error_does_not_cancel_intent(patched):
    ...
    result = asyncio.run(
        reconcile_mod.reconcile_account(
            account_id=1,
...
def test_rate_limit_error_does_not_cancel_intent(patched):
...
def test_binance_2013_marks_canceled(patched):
...
def test_clean_empty_history_marks_canceled(patched):
...
def test_live_order_found_is_not_canceled(patched):
