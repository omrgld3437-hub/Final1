...
def test_daily_ref_sticky_within_tr_day(tmp_path):
    ...
    with patch("app.services.test_account_kpi._REF_ROOT", tmp_path), patch(
        "app.utils.tz_utils.turkey_today_date_str", return_value="2026-05-29"
    ):
...
def test_spot_strip_total_sums_avail_bot_locked():
    ...
    with patch(
        ...
        total, avail, bot, locked = compute_test_account_spot_strip_total_usd(wallet, db, 2)
    assert avail == 7500.0
    assert bot == 2487.01
    assert locked == 0.0
