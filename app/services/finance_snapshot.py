...
try:
    from app.services.binance_assets import get_account_keys, fetch_prices_map, _convert_to_usd
except ImportError:
...
class SnapshotService:
    ...
    async def create_snapshot(self, account_id: int, source: str = "rest_snapshot") -> Optional[AssetSnapshot]:
        """
        ...
        try:
            ...
            if is_test_account(account_id, self.db):
                ...
                try:
                    ...
                    pass
                price_map = {sym: float(d.get("price") or 0) for sym, d in prices.items()} if prices else {}
                usd = price_map.get("USDT") or 1.0
                total_usd = float(TEST_PAPER_BALANCE_USDT) * usd
                breakdown = {"USDT": {"free": TEST_PAPER_BALANCE_USDT, "locked": 0, "total": TEST_PAPER_BALANCE_USDT, "usdValue": total_usd, "priceUsed": usd}}
                ...
                logger.info(f"[Snapshot] Created paper snapshot for test account {account_id}: ${total_usd:.2f}")
                return snapshot
        ...
        try:
            ...
            if not get_account_keys or not fetch_prices_map or not _convert_to_usd:
                logger.warning(f"[Snapshot] Binance services not available, skipping snapshot for account {account_id}")
                return None
            ...
            try:
                ...
                logger.warning(f"[Snapshot] binance_spot not available, skipping snapshot for account {account_id}")
                return None
            
            keys = await get_account_keys(account_id, self.db)
            if not keys:
                logger.warning(f"[Snapshot] Could not get keys for account {account_id}")
                return None
            ...
            for balance in balances:
                ...
                price_used = prices.get(f"{asset}USDT") or prices.get(f"{asset}BUSD") or 0.0
            ...
            logger.info(f"[Snapshot] Created snapshot for account {account_id}: ${total_usd:.2f}")
            return snapshot
            
        except Exception as e:
            logger.error(f"[Snapshot] Error creating snapshot for account {account_id}: {e}")
            self.db.rollback()
            return None
    ...
    def get_equity_curve(self, account_id: int, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> List[Dict]:
        """
        ...
        return [
            {
                ...
                "total_usd_value": s.total_usd_value
            }
            for s in snapshots
        ]
