    ...
    try:
        ...
        if bots:
            try:
                ...
                for row in rows:
                    ...
                    last_trade_by_bot[bot_id] = last_ts.isoformat() + "Z" if last_ts else None
            except Exception as e:
        ...
        for bot in bots:
            ...
            if not pnl_data.get("error"):
                total_profit_usd += pnl_data.get("realized", 0.0)
        ...
        account_kpis = {
            ...
            "total_pnl_usd": round(total_profit_usd, 2),
...
async def fetch_finance_pnl(account_id: int, db: Session) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "today": {"pnl": pnl_today["realized_pnl"], "fees": pnl_today["fees"], "trades_count": pnl_today["trades_count"]},
            "last_7d": {"pnl": pnl_7d["realized_pnl"], "fees": pnl_7d["fees"], "trades_count": pnl_7d["trades_count"]},
            "last_30d": {"pnl": pnl_30d["realized_pnl"], "fees": pnl_30d["fees"], "trades_count": pnl_30d["trades_count"]},
