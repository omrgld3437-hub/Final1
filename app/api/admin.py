...
_ADMIN_ACCOUNTS_LIVE_CONCURRENCY = max(
    1, int(os.getenv("ADMIN_ACCOUNTS_LIVE_CONCURRENCY", "4"))
)
_ADMIN_ACCOUNTS_FULL_CACHE_TTL_SEC = max(
    0.0, float(os.getenv("ADMIN_ACCOUNTS_FULL_CACHE_TTL_SEC", "15"))
)
    ...
    try:
        ...
        if is_test_account(account_id, db):
            from app.services.test_account_kpi import (
                compute_test_account_dashboard_spot_kpi_async,
            )
        ...
        resp = _wallet_response(
            account_id, balances, price_map, bot_locked=bot_locked_map
        )
        return (
...
async def _admin_fetch_live_row_kpis(
    account_id: int, acct_is_test: bool
) -> Dict[str, Any]:
    ...
    try:
        if acct_is_test:
            ...
            test_spot_kpi = await compute_test_account_dashboard_spot_kpi_async(
                account_id, db
            )
            ...
            (
                ...
                bot_locked_usd,
                spot_balance_status,
            ) = await _get_account_wallet_strip_kpis(account_id, db)
...
async def get_admin_accounts(
    ...
    if not lite and work_account_ids:
        live_by_account = await _admin_fetch_live_rows_parallel(
            work_account_ids, test_account_ids
        )
        for aid, live in live_by_account.items():
            ...
            if live.get("wallet_ms", 0) >= 2500:
                slow_wallets.append(
                    {"account_id": aid, "wallet_ms": round(live["wallet_ms"])}
                )
            if live.get("bot_kpi_error"):
                bot_kpi_errors.append(
                    {"account_id": aid, "error": live["bot_kpi_error"]}
                )
    ...
    for account in work_accounts:
        try:
            ...
            if acct_is_test and lite:
                ...
                spot_balance, bot_locked_usd, spot_balance_status = (
                    _get_lite_wallet_kpis(account.id, db)
                )
            else:
            ...
            if not lite:
                ...
                try:
                    if not bot_raw.get("_error"):
                        ...
                        daily_sum = sum(
                            float(b.get("daily_pnl_usd") or 0) for b in bots_array
                        )
                        ...
                        if ref_sum > 1e-9:
                            ...
                            daily_pnl_pct = (
                                (daily_pnl_usd / ref_open * 100.0)
                                if ref_open > 1e-9
                                else 0.0
                            )
                except Exception as bot_kpi_ex:
                    bot_kpi_errors.append(
                        {"account_id": account.id, "error": str(bot_kpi_ex)[:300]}
                    )
                    logger.warning(
            ...
            if acct_is_test and test_spot_kpi:
                daily_wallet_pnl_usd = float(
                    test_spot_kpi.get("daily_wallet_pnl_usd") or 0.0
                )
                daily_wallet_pnl_pct = float(
                    test_spot_kpi.get("daily_wallet_pnl_pct") or 0.0
                )
            else:
                try:
                    last_before_today = (
                    ...
                    if (
                        last_before_today
                        and getattr(last_before_today, "total_usd_value", None)
                        is not None
                    ):
                        ...
                        if (
                            first_today
                            and getattr(first_today, "total_usd_value", None)
                            is not None
                        ):
                    ...
                    if not user.is_admin:
                        if user.last_login_at:
                            user_login_info = (
                                ...
                                else str(user.last_login_at)
                            )
                        if user.last_logout_at:
                            user_logout_info = (
                                user.last_logout_at.isoformat()
                                if hasattr(user.last_logout_at, "isoformat")
                                else str(user.last_logout_at)
                            )
                        # last_activity_at: her 60s ping ile güncellenir — giriş saatinden daha güncel
                        if getattr(user, "last_activity_at", None):
                            user_activity_info = (
                                user.last_activity_at.isoformat()
                                if hasattr(user.last_activity_at, "isoformat")
                                else str(user.last_activity_at)
                            )
            ...
            accounts_list.append(
                {
                    ...
                    if daily_wallet_pnl_usd is not None
                    else None,
                    "daily_wallet_pnl_pct": round(daily_wallet_pnl_pct, 2)
                    if daily_wallet_pnl_pct is not None
                    else None,
                    "admin_isolated": bool(
                        getattr(account, "isolate_from_admin", False)
                    ),
    ...
    try:
        ...
        logger.warning(
            "[Admin] Error sorting accounts: %s request_id=%s", e, request_id
        )

    duration_ms = (time.perf_counter() - t0) * 1000.0
    log_level = (
        logger.warning
        if duration_ms >= 3000 or wallet_errors or bot_kpi_errors or row_errors
        else logger.info
    )
    log_level(
    ...
    try:
        ...
        if not lite and _ADMIN_ACCOUNTS_FULL_CACHE_TTL_SEC > 0:
            async with _get_admin_accounts_full_cache_lock():
                ...
                if len(_admin_accounts_full_cache) > 32:
                    ...
                    for k in [
                        x
                        for x, v in _admin_accounts_full_cache.items()
                        if v[0] < cutoff
                    ]:
        ...
        logger.error(
            "[Admin] Error building response: %s request_id=%s",
            e,
            request_id,
            exc_info=True,
        )
...
async def delete_admin_account(
    ...
    try:
        ...
        if bots:
            bot_count = len(bots)
            logger.warning(
                f"[{trace_id}] Cannot delete account {account_id}: has {bot_count} bots"
            )
            raise HTTPException(
                status_code=400,
        ...
        if account.user_id:
            ...
            if user:
                ...
                logger.info(
                    f"[{trace_id}] User {user.id} ({user.username}) soft deleted"
                )
...
def _install_psutil_sync() -> bool:
    ...
    try:
        ...
        if r.returncode != 0:
            logger.warning(
                "pip install psutil failed: %s",
                (r.stderr or r.stdout or b"").decode("utf-8", "replace")[:500],
            )
            return False
...
async def get_error_logs(
    ...
    max_unique: int = Query(
        50, ge=1, le=200, description="Maksimum benzersiz hata sayısı"
    ),
...
async def get_server_stats(current: dict = Depends(_require_admin)) -> Dict:
    ...
    try:
        ...
        mem_mb, mem_total_mb, cpu_percent, network_mbps_down, network_mbps_up = (
            _collect_psutil_stats()
        )
...
async def admin_force_unlock_symbol(
    ...
    return {
        "success": True,
        "cleared": count,
        "message": f"Cleared {count} expired lock(s) for {symbol}"
        if count
        else f"No expired lock for {symbol}",
    }
...
async def server_lockdown(current: dict = Depends(_require_admin)) -> Dict:
    ...
    return {
        "success": True,
        "lockdown": True,
        "message": "Erişim kapatıldı. Sadece admin erişebilir.",
    }
...
async def server_exit(
    ...
    if not password:
        raise HTTPException(
            status_code=400,
            detail="Şifre girin. Sunucuyu kapatmak için admin şifrenizi girin.",
        )
    ...
    if not stored_hash or not isinstance(stored_hash, str) or len(stored_hash) < 10:
        raise HTTPException(
            status_code=403,
            detail="Admin şifresi tanımlı değil. Önce ayarlardan şifre belirleyin.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Şifre doğrulanamadı. Lütfen doğru admin şifresini girin.",
        )

    if not password_valid:
        logger.warning(
            "Server exit rejected: wrong password for user_id=%s",
            current.get("user_id"),
        )
        raise HTTPException(
            status_code=400,
            detail="Yanlış şifre. Sunucuyu kapatmak için admin şifrenizi girin.",
        )
...
async def server_restart(
    ...
    if not password:
        raise HTTPException(
            status_code=400,
            detail="Şifre girin. Sunucuyu yeniden başlatmak için admin şifrenizi girin.",
        )
    user = db.query(User).filter(User.id == current["user_id"]).first()
    if not user:
        raise HTTPException(status_code=403, detail="Kullanıcı bulunamadı.")
    ...
    if not password_valid:
        logger.warning(
            "Server restart rejected: wrong password for user_id=%s",
            current.get("user_id"),
        )
        raise HTTPException(
            status_code=400,
            detail="Yanlış şifre. Sunucuyu yeniden başlatmak için admin şifrenizi girin.",
        )
    logger.warning(
        "Server restart confirmed by admin (password OK) user_id=%s",
        current.get("user_id"),
    )
    import os
    import sys
    ...
    if is_windows:
        ...
        if not os.path.isfile(restart_script):
            ...
            raise HTTPException(
                status_code=500,
                detail="Restart script bulunamadı (scripts/restart_server_win.py).",
            )
        ...
        if not os.path.isfile(restart_script):
            ...
            raise HTTPException(
                status_code=500,
                detail="Restart script bulunamadı (scripts/restart_server.py).",
            )
        if not os.path.isfile(run_sh):
            logger.error(
                "Başlatma scripti bulunamadı (run.sh, start.command, start): %s", run_sh
            )
            raise HTTPException(
                status_code=500,
                detail="run.sh veya start.command/start bulunamadı. Proje köküne run.sh ekleyin veya start.command kullanın.",
            )
        run_dir = os.path.join(project_root, ".run")
        os.makedirs(run_dir, exist_ok=True)
        log_path = os.path.join(run_dir, "restart_helper.log")
    ...
    logger.warning(
        "Server restart scheduled by admin - helper PID %s, exit in 5 seconds", proc.pid
    )
...
async def get_admin_popup_detail(
    ...
    return {
        "popup": {
            ...
            "valid_until": popup.valid_until.isoformat()
            if hasattr(popup.valid_until, "isoformat")
            else str(popup.valid_until),
            "created_at": popup.created_at.isoformat()
            if popup.created_at and hasattr(popup.created_at, "isoformat")
            else str(popup.created_at or ""),
            "is_active": popup.valid_until >= now
            if hasattr(popup.valid_until, "__ge__")
            else True,
        ...
        "dismissals": [
            {
                ...
                if d.dismissed_at and hasattr(d.dismissed_at, "isoformat")
                else str(d.dismissed_at or ""),
            }
        ...
        items.append(
            {
                ...
                else str(p.created_at or ""),
                "is_active": p.valid_until >= now
                if hasattr(p.valid_until, "__ge__")
                else True,
...
async def create_admin_popup(
    ...
    if req.target not in ("first_login", "normal_user"):
        raise HTTPException(
            status_code=400, detail="target first_login veya normal_user olmalı"
        )
    if (req.title_key or "").strip() not in POPUP_TITLE_KEYS:
        raise HTTPException(
            status_code=400,
            detail="title_key şunlardan biri olmalı: " + ", ".join(POPUP_TITLE_KEYS),
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Geçerlilik süresi geçerli bir tarih olmalı (örn. 2026-02-15T23:59:00)",
        )
    if valid_until.tzinfo:
        valid_until = valid_until.astimezone(timezone.utc).replace(tzinfo=None)
    if valid_until <= datetime.utcnow():
        raise HTTPException(
            status_code=400, detail="Geçerlilik süresi gelecekte bir tarih olmalı"
        )
...
async def delete_admin_popup(
    ...
    if not popup:
        raise HTTPException(status_code=404, detail="Pop-up bulunamadı")
    db.query(AdminPopupDismissal).filter(
        AdminPopupDismissal.popup_id == popup_id
    ).delete()
    db.delete(popup)
    db.commit()
