...
def _sort_and_cap_assets(assets: List[Dict], max_n: int) -> List[Dict]:
    """Sort by usdt_value desc, cap to max_n. Put null usdt_value at end."""
    ...
    if account_id in _fast_cache:
        ...
        if entry.get("expires_at", 0) > now_mono:
            ...
            logger.info(
                ...
                account_id, request_id, server_ms, len(payload_bytes),
            )
            return {
                "ok": True,
                "data": entry.get("data", {}),
...
async def _do_wallet_refresh(account_id: int, db: Session, request_id: str, force: bool) -> Dict[str, Any]:
    ...
    max_assets = cfg.get("home_fast_max_assets", 20)
    try:
        wallet_raw = await asyncio.wait_for(_fetch_wallet_uncached(account_id, db), timeout=12.0)
        ...
        if _is_rate_limit_error(e):
            _wallet_last_error_code[account_id] = "BINANCE_RATE_LIMIT"
            _wallet_cooldown_until[account_id] = time.monotonic() + cfg.get("wallet_cooldown_sec", 30)
    ...
    minimal["ts"] = ts_iso
    _in_memory_wallet[account_id] = (minimal, time.monotonic())
    return minimal
...
async def home_wallet_refresh(
    ...
    # Cached response for skip cases
    ...
    try:
        result = await asyncio.wait_for(task, timeout=14.0)
    except asyncio.TimeoutError:
...
async def home_wallet_status(
    ...
    """Return inflight, last_live_at, cooldown_until, last_error_code (no Binance call)."""
    request_id = getattr(request.state, "request_id", None) or ""
    require_account_access(current, account_id)
    cfg = get_config()
    cooldown_sec = cfg.get("wallet_cooldown_sec", 30)
