...
FINANCE_TRADES_ALL_MAX_ROWS = 5000  # type_filter=all tek istekte en fazla bu kadar fill çekilir
...
async def get_finance_summary(
    ...
    try:
        ...
        initial_value = 0.0
        first_snapshot = db.query(AssetSnapshot).filter(
            AssetSnapshot.account_id == account_id
        ...
        if first_snapshot:
            ...
            for bot in bots:
                try:
                    ...
                    initial_value += float(config.get("budget_usd") or config.get("bot_budget_quote") or 0)
                except:
                    pass
        ...
        try:
            last_wallet_before_today = db.query(AssetSnapshot).filter(
                AssetSnapshot.account_id == account_id,
                AssetSnapshot.timestamp < today_start
            ).order_by(desc(AssetSnapshot.timestamp)).first()
            if last_wallet_before_today and total_usd_value is not None:
                wallet_start_of_day_usd = float(last_wallet_before_today.total_usd_value)
                daily_wallet_pnl_usd = float(total_usd_value) - wallet_start_of_day_usd
            else:
                first_snap_today = db.query(AssetSnapshot).filter(
                    AssetSnapshot.account_id == account_id,
                    AssetSnapshot.timestamp >= today_start
                ).order_by(AssetSnapshot.timestamp.asc()).first()
...
async def get_finance_report(
    ...
    # Calculate metrics
    trades = db.query(TradeNormalized).filter(
    ...
    win_rate = len([s for s, d in pnl_data["by_symbol"].items() if d["pnl"] > 0]) / len(pnl_data["by_symbol"]) if pnl_data["by_symbol"] else 0.0
...
def _clean_deposit_withdraw_cache():
    ...
    now = time.monotonic()
    to_del = [k for k, v in _deposit_withdraw_cache.items() if len(v) >= 3 and v[2] < now]
    for k in to_del:
        _deposit_withdraw_cache.pop(k, None)
    if len(_deposit_withdraw_cache) > _DEPOSIT_WITHDRAW_CACHE_MAX_KEYS:
        by_expiry = sorted(_deposit_withdraw_cache.items(), key=lambda x: (x[1][2] if len(x[1]) >= 3 else 0))
        for k, _ in by_expiry[: len(_deposit_withdraw_cache) - _DEPOSIT_WITHDRAW_CACHE_MAX_KEYS]:
            _deposit_withdraw_cache.pop(k, None)
...
async def _fetch_deposit_withdraw(
    ...
    async with httpx.AsyncClient(timeout=20.0) as client:
        try:
            ...
            data = await _signed_request(client, "GET", "/sapi/v1/capital/deposit/hisrec", keys, params)
            ...
            if is_transient_upstream_error(e):
                ...
                if getattr(e, "response", None) and getattr(e.response, "status_code", None) in (400, 401):
                    ...
                    _log.warning("deposit hisrec error account_id=%s: %s", account_id, e)
        try:
            params = {"startTime": start_ms, "endTime": end_ms, "limit": 1000}
            data = await _signed_request(client, "GET", "/sapi/v1/capital/withdraw/history", keys, params)
            if isinstance(data, list):
                withdrawals = data
            elif isinstance(data, dict) and "data" in data:
            ...
            if is_transient_upstream_error(e):
                ...
                if getattr(e, "response", None) and getattr(e.response, "status_code", None) in (400, 401):
                    ...
                    _log.warning("withdraw history error account_id=%s: %s", account_id, e)
...
async def get_finance_trades(
    ...
    type_filter: Optional[str] = Query(None, description="all | buysell | depositwithdraw"),
    ...
    sync: int = Query(0, description="1 = sync trades from Binance and fetch deposit/withdraw"),
    db: Session = Depends(get_db),
    current: dict = Depends(require_auth),
) -> Dict:
    """
    ...
    try:
        ...
        if sync:
            ...
            if now - last < TRADE_SYNC_COOLDOWN_SEC:
                logger.info("[finance/trades] sync skipped (cooldown) account_id=%s", account_id)
            else:
                async with _trade_sync_lock:
                    ...
                    if now - last < TRADE_SYNC_COOLDOWN_SEC:
                        ...
                        try:
                            ...
                            logger.warning("[finance/trades] sync_account_trades failed: %s", e)
        ...
        if start_time is None and end_time is None:
            ...
            end_time = now_utc
            default_days = FINANCE_TRADES_ALL_DEFAULT_DAYS if (tf in ("all", "")) else FINANCE_TRADES_DEFAULT_DAYS
        ...
        total_query = (
            db.query(
                func.count(
                    distinct(
                        case(
                            (TradeNormalized.order_id.isnot(None), TradeNormalized.order_id),
        ...
        order_key_col = func.coalesce(TradeNormalized.order_id, TradeNormalized.trade_id)

        if use_db_pagination:
            page_keys_query = (
                db.query(order_key_col.label("ok"), func.max(TradeNormalized.time).label("mt"))
                .filter(
                    TradeNormalized.account_id == account_id,
                    TradeNormalized.side.in_(["BUY", "SELL"]),
                )
            ...
            if start_time is not None:
                page_keys_query = page_keys_query.filter(TradeNormalized.time >= start_time)
            if end_time is not None:
                page_keys_query = page_keys_query.filter(TradeNormalized.time <= end_time)
            if symbol:
                page_keys_query = page_keys_query.filter(TradeNormalized.symbol == symbol.upper())
            if bot_id:
                page_keys_query = page_keys_query.filter(TradeNormalized.bot_id == bot_id)
            if side:
                page_keys_query = page_keys_query.filter(TradeNormalized.side == side.upper())
            page_keys_query = (
        ...
        def convert_commission_to_usd(commission: float, commission_asset: str) -> float:
            if commission <= 0:
                return 0.0
        ...
        for fill in all_fills:
            ...
            fill_commission_usd = convert_commission_to_usd(fill.commission, fill.commission_asset or "USDT")
        ...
        for order_key, order in orders_map.items():
            ...
            orders_list.append({
                ...
                "time": _utc_iso(order["last_fill_time"]),  # Use last fill time for sorting
        ...
        if tf == "buysell":
            paginated_orders = orders_list if use_db_pagination else orders_list[offset : offset + limit]
            return {
                "total": total_orders,
                "limit": limit,
                "offset": offset,
...
async def trigger_finance_sync(
    ...
    if sync_type in ["snapshot", "all"]:
        snapshot_service = SnapshotService(db)
        snapshot = await snapshot_service.create_snapshot(account_id, source="manual_sync")
        results["snapshot"] = {
