...
async def create_bot_v2(
    ...
    if request_body.ref_price_mode == "market_now" or not ref_price:
        ...
        if request_body.mode == "live":
            ...
            adapter = BinanceSpotAdapterV2(
                request_body.account_id, "live", api_key, api_secret
            )
        else:
    ...
    for idx, grid_data in enumerate(request_body.up_grids):
        trigger_abs = (
            ref_price * (1 + grid_data["trigger_value"] / 100.0)
            if grid_data.get("trigger_type") == "PCT"
            else grid_data["trigger_value"]
        )
        grid = BotGridV2(
            bot_id=bot.id,
            side="UP_SELL",
    ...
    cycle = BotCycleV2(
        bot_id=bot.id, cycle_no=1, start_ts=datetime.utcnow(), status="OPEN"
    )
    db.add(cycle)

    # Create state
    state = BotStateV2(bot_id=bot.id, state_json='{"last_price": 0, "ref_price": 0}')
    db.add(state)
    ...
    config_summary = {
        "symbol": request_body.symbol,
        "mode": request_body.mode,
        "budget_usdt": request_body.budget_usdt,
        "base_alloc_pct": request_body.base_alloc_pct,
        "up_grids_count": len(request_body.up_grids),
