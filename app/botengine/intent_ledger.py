...
def upsert_intent(
    ...
    existing = db.execute(
        text(
            "SELECT id, intent_id, client_order_id, binance_order_id, status FROM order_intents WHERE intent_id = :iid"
        ),
        {"iid": intent_id},
    ).fetchone()
    if existing:
        return {
            "id": existing[0],
            "intent_id": existing[1],
            "client_order_id": existing[2],
            "binance_order_id": existing[3],
            "status": existing[4],
        }, False
...
def get_intent_by_client_order_id(
    db: Session, client_order_id: str
) -> Optional[Dict[str, Any]]:
    """Get intent by client_order_id for reconciliation."""
    r = db.execute(
        text(
            "SELECT id, intent_id, bot_id, account_id, symbol, side, qty, client_order_id, binance_order_id, status FROM order_intents WHERE client_order_id = :coid"
        ),
        {"coid": client_order_id},
    ).fetchone()
    if not r:
...
def update_intent_sent(db: Session, intent_id: str) -> bool:
    ...
    r = db.execute(
        text(
            "UPDATE order_intents SET status = 'SUBMITTED', updated_at = :now WHERE intent_id = :iid"
        ),
    ...
    if r.rowcount == 0:
        r = db.execute(
            text(
                "UPDATE order_intents SET status = 'SENT', updated_at = :now WHERE intent_id = :iid"
            ),
            {"now": now, "iid": intent_id},
        )
    db.commit()
...
def update_intent_submitting(
    db: Session, intent_id: str, submit_ts: Optional[float] = None
) -> bool:
...
def update_intent_from_binance(
    ...
    final_ts = (
        time.time()
        if status in (STATUS_FILLED, STATUS_CANCELED, STATUS_REJECTED)
        else None
    )
    r = db.execute(
        text("""
...
def get_intent_status(db: Session, intent_id: str) -> Optional[str]:
    ...
    r = db.execute(
        text("SELECT status FROM order_intents WHERE intent_id = :iid"),
        {"iid": intent_id},
    ).fetchone()
...
def get_non_final_intents_for_account(
...
def _bot_has_pending_intents(db: Session, bot_id: int) -> bool:
    ...
    row = db.execute(
        text(
            "SELECT 1 FROM order_intents WHERE bot_id = :bid AND status = 'PENDING' LIMIT 1"
        ),
        {"bid": bot_id},
    ).fetchone()
...
async def reconcile_open_orders_for_bot(
    adapter: Any, bot_id: int, account_id: int, db: Session, symbol: str
) -> int:
    """
    ...
    try:
        ...
        for o in open_orders or []:
            ...
            row = db.execute(
                text(
                    "SELECT id, intent_id, status FROM order_intents WHERE bot_id = :bid AND client_order_id = :coid"
                ),
            ...
            if row and row[2] == "PENDING":
                ...
                updated += 1
                log.info(
                    "RECONCILE_OPEN_ORDER bot_id=%s client_order_id=%s intent=%s",
                    bot_id,
                    coid,
                    row[1],
                )
        ...
        if isinstance(e, BinanceIPBannedError):
            log.debug(
                "reconcile_open_orders_for_bot bot_id=%s skipped (IP ban until %.0f)",
                bot_id,
                e.banned_until_ts,
            )
        elif is_transient_upstream_error(e):
            log.debug(
                "reconcile_open_orders_for_bot bot_id=%s transient err=%s", bot_id, e
            )
        else:
