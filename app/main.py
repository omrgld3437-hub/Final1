...
console_handler.setLevel(logging.WARNING if getattr(sys.stdout, "isatty", lambda: False)() else logging.INFO)
...
class WebSocketCloseFilter(logging.Filter):
    ...
    _SUPPRESS_NAMES = frozenset({
        "ConnectionClosedOK", "ConnectionClosedError", "CancelledError",
        "IncompleteReadError", "ConnectionClosed",
    })
    ...
    def filter(self, record):
        ...
        if any(k in msg for k in (
            "ConnectionClosedOK", "ConnectionClosedError", "ConnectionClosed",
            "no close frame", "sent 1012", "service restart",
            "IncompleteReadError", "0 bytes read", "CancelledError",
        )):
            return False
        if "ASGI application" in msg and (
            "websocket" in msg.lower() or "ConnectionClosed" in msg
            or "IncompleteRead" in msg or "CancelledError" in msg
        ):
            return False
        return True
...
botengine_console.setLevel(logging.WARNING if getattr(sys.stdout, "isatty", lambda: False)() else logging.INFO)
...
async def startup_event():
    ...
    async def _audit_server_start():
        try:
            ...
            def _sync():
                ...
                try:
                    audit_svc.log_event(
                        _db, actor_type="system", event_type="SERVER_START", severity="INFO",
            ...
            def _sync():
                ...
                try:
                    ...
                    if admin:
                        ...
                        _db.commit()
                        logger.info("First-admin: Admin zaten var (kullanici adi: %s)", _USERNAME)
                        return
                    account = Account(
                    ...
                    _db.commit()
                    logger.info("First-admin: Ilk admin olusturuldu (Admin). Ilk girisinde yazacagi sifre kalici olacak.")
                finally:
                    _db.close()
            await loop.run_in_executor(None, _sync)
        except Exception as e:
    ...
    try:
        ...
        await asyncio.wait_for(data_hub.warmup(warmup_timeout), timeout=warmup_timeout + 1.0)
    except Exception as e:
...
async def request_metrics_middleware(request, call_next):
        ...
        try:
            ...
            try:
                persist_error(
                    ...
                    path=path, method=method, request_id=request_id,
                    user_id=user_id, account_id=account_id,
                    user_agent=(user_agent or "")[:512], client_ip=client_ip or None,
    ...
    if 200 <= status < 300:
        try:
            ...
            from app.db.session import SessionLocal
            auth = request.headers.get("Authorization") or request.headers.get("authorization") or ""
            token = (auth[7:].strip() if auth.startswith("Bearer ") else None) or request.cookies.get("auth_token")
            session = None
            if token:
                _db = SessionLocal()
                try:
            ...
            if requested_account_id is None and request.scope.get("query_string"):
                q = request.scope.get("query_string", b"").decode("utf-8", errors="ignore")
                for part in q.split("&"):
            ...
            account_scoped_prefixes = ("/api/accounts/", "/api/dashboard/summary", "/api/bots-engine", "/api/bots/", "/api/finance/")
            ...
            if (
                ...
                if sid is not None and int(sid) != int(requested_account_id):
                    ...
                    logger.critical(
                        ...
                        path, requested_account_id, sid, session.get("user_id"), client_ip,
                    )
                    try:
                        from app.db.session import SessionLocal
                        ...
                        try:
                            ...
                            if uid:
                                ...
                                if u and not getattr(u, "is_admin", False):
                                    u.is_suspended = True
                                    db.commit()
                                    logger.warning("BREACH: user_id=%s askıya alındı.", uid)
                        finally:
                            db.close()
                    except Exception as e:
...
async def lockdown_middleware(request, call_next):
    ...
    if auth_header.startswith("Bearer "):
        ...
        try:
            ...
            pass
    logger.warning("Lockdown: blocking path (whitelist'te yok) path=%s", request.url.path)
        ...
        try:
            ...
            try:
                ...
                if isinstance(exc, (ConnectionClosedOK, ConnectionClosedError)):
                    ...
                    if code in (1000, 1001, 1006, 1012) or code is None:  # normal / going away / abnormal / restart
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    if forwarded:
        client_ip = (forwarded.split(",")[0].strip() or client_ip)
    user_agent = (request.headers.get("user-agent") or "")[:512] if request.headers else None
    user_id, account_id, is_admin = None, None, False
    try:
        auth = request.headers.get("Authorization") or request.headers.get("authorization") or ""
        token = (auth[7:].strip() if auth.startswith("Bearer ") else None)
    ...
    try:
        from websockets.exceptions import ConnectionClosedOK, ConnectionClosedError
        if isinstance(exc, (ConnectionClosedOK, ConnectionClosedError)) and getattr(exc, "code", 1000) in (1000, 1001):
            logger.debug("WebSocket normal close, skip persist.")
            return JSONResponse(status_code=200, content={})
    except ImportError:
        pass
    if "No response returned" in (str(exc) or ""):
        logger.debug("Unhandled No response returned (client disconnect), skip persist.")
    else:
        try:
            from app.db.session import SessionLocal
            from app.error_logging import persist_error
            ...
            try:
                persist_error(
                    db, "backend", str(exc) or type(exc).__name__,
                    detail=traceback.format_exc(),
                    path=path, method=method, request_id=request_id,
                    user_id=user_id, account_id=account_id,
                    user_agent=user_agent, client_ip=client_ip,
                    is_admin=is_admin, level="error",
                )
            finally:
                db.close()
        except Exception as e:
...
def _format_http_detail(status: int, detail) -> tuple:
    ...
    """
    status_label = {401: "Unauthorized", 403: "Forbidden", 404: "Not Found", 429: "Too Many Requests"}.get(status, f"HTTP {status}")
    if isinstance(detail, dict):
        ...
        return message_for_log[:500], {k: v for k, v in context.items() if v is not None}
    # str veya diğer
            ...
            try:
                ...
                try:
                    persist_error(
                        ...
                        user_agent=user_agent, client_ip=client_ip,
                        context=context,
                        is_admin=is_admin, level="error" if status >= 500 else "warning",
                    )
                finally:
                    db.close()
            except Exception as e:
...
if not _home_routes_loaded:
    ...
    async def _fallback_home_fast(
        ...
        return {
            "ok": True,
            "data": {"prices": {}, "kpis": {}, "wallet_cached": None, "wallet_cached_at": None, "wallet_live_inflight": False},
            "meta": {"request_id": "", "server_ms": 0, "payload_bytes": 0, "cache": False, "stale": True, "generated_at": None},
        }
...
async def api_admin_create_test_error(request: Request):
    ...
    token = (auth[7:].strip() if auth.startswith("Bearer ") else None)
    if not token:
        raise HTTPException(status_code=401, detail={"message": "Yetkilendirme gerekli"})
    session = _session_get(token)
    if not session or not session.get("is_admin"):
        raise HTTPException(status_code=403, detail={"message": "Bu işlem için admin yetkisi gerekli"})
    db = SessionLocal()
    try:
        rid = persist_error(
            db,
        ...
        if grouped:
            ...
            for _key, group in groups:
                ...
                if getattr(rep, "context_json", None):
                    try:
                        ctx = json.loads(rep.context_json) if isinstance(rep.context_json, str) else rep.context_json
                    except Exception:
                        ctx = None
                errors.append({
                    "id": rep.id,
                    ...
                    "created_at": rep.created_at.isoformat() + "Z" if rep.created_at else None,
...
from app.api.admin import list_admin_popups, create_admin_popup, delete_admin_popup, get_admin_popup_detail, _require_admin, CreatePopupRequest
...
async def api_health_marketdata():
    ...
    try:
        from app.services.data_hub import data_hub
        prices_ready = bool(getattr(data_hub, "prices", None) and len(data_hub.prices) > 0)
        return {"ok": True, "prices_ready": prices_ready, "prices_count": len(getattr(data_hub, "prices", {}))}
    except Exception:
...
async def api_health_ram():
    ...
    if os.getenv("RAM_PROBE", "").strip() != "1" and os.getenv("RAM_PROBE_ENABLED", "").strip() != "1":
        from fastapi.responses import JSONResponse
...
async def ui_admin_html(request: Request):
    ...
    try:
        ...
        return RedirectResponse(url="/ui/login.html", status_code=302)
    return FileResponse(UI_DIR / "admin.html", headers={
...
async def ui_dashboard():
    ...
    return FileResponse(UI_DIR / "dashboard.html", headers={
        "Cache-Control": "no-store, no-cache, must-revalidate, max-age=0",
        "Pragma": "no-cache",
        "Expires": "0",
    })
...
async def trader_trailing():
    """Kavram sayfası"""
    return FileResponse(UI_DIR / "trader-trailing.html", headers={
...
async def robots_txt():
    """robots.txt"""
    return FileResponse(UI_DIR / "robots.txt", media_type="text/plain", headers={
...
async def sitemap_xml():
    """sitemap.xml"""
    return FileResponse(UI_DIR / "sitemap.xml", media_type="application/xml", headers={
        "Cache-Control": "public, max-age=86400",
    })
...
async def ui_login():
    """Serve login.html"""
    return FileResponse(UI_DIR / "login.html", headers={
        "Cache-Control": "no-store, no-cache, must-revalidate, max-age=0",
        "Pragma": "no-cache",
        "Expires": "0",
    })
