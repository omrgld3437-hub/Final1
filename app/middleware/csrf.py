...
async def csrf_middleware(request: Request, call_next):
    ...
    if cfg.get("auth_csrf_origin_check"):
        if origin:
            if not _origin_ok(origin, allowed):
                ...
                logger.warning(
                    "CSRF Origin mismatch request_id=%s path=%s origin=%s",
                    rid,
                    path,
                    origin[:80],
                )
            ...
            logger.debug(
                "CSRF: Origin and Referer missing (allowed when not strict) request_id=%s",
    ...
    if cfg.get("auth_csrf_double_submit") and not origin_only:
        ...
        if not csrf_cookie or not csrf_header or csrf_cookie != csrf_header:
            rid = getattr(request.state, "request_id", None)
            logger.warning(
                "CSRF double-submit mismatch request_id=%s path=%s", rid, path
            )
            return JSONResponse(
                status_code=403,
                content={
