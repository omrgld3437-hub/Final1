...
class BotManager:
    ...
    def __init__(self):
        self._bots: Dict[
            Tuple[int, int], Union[BotEngine, "DCABotEngineV3"]
        ] = {}  # (account_id, bot_id) -> Engine
        self._bot_types: Dict[
            Tuple[int, int], str
        ] = {}  # (account_id, bot_id) -> bot_type

    def get_bot(
        self, bot_id: int, account_id: int
    ) -> Optional[Union[BotEngine, "DCABotEngineV3"]]:
    ...
    def register_bot(
        ...
        bot_id: int,
        account_id: int,
        engine: Union[BotEngine, "DCABotEngineV3"],
        bot_type: str = "legacy",
    ):
        """Register bot engine"""
        key = (account_id, bot_id)
    ...
    def unregister_bot(self, bot_id: int, account_id: int):
        ...
        if key in self._bots:
            ...
            if bot_type == "dca":
                ...
                logger.info(
                    f"Unregistering DCA V3 bot {bot_id} - worker will handle cleanup"
                )
    ...
    def load_bot_from_db(
        self, db: Session, bot_id: int, account_id: int
    ) -> Optional[Union[BotEngine, "DCABotEngineV3"]]:
    ...
    def _load_dca_v3_bot(
        self, bot: Bot, config_data: dict, db: Session
    ) -> Optional["DCABotEngineV3"]:
        """Load DCA V3 bot"""
    ...
    def _load_legacy_bot(
        ...
        logger.debug(
            f"[load_bot_from_db] Bot {bot_id}: filtered_data keys: {list(filtered_data.keys())}"
        )
        ...
        try:
            ...
            if not base_asset_val:
                ...
                sym = symbol_val
                base_asset_val = (
                    sym.replace("USDT", "").replace("BUSD", "").replace("FDUSD", "")
                )
            ...
            logger.error(
                "[BotConfig Error] BotConfig fields expected: symbol, base_asset, quote_asset, grid_count, upper_price, lower_price, order_amount, mode"
            )
            raise
        ...
        try:
            logger.debug(
                f"[BotEngine] Creating engine: bot_id={bot_id}, account_id={account_id}, mode={config.mode}"
            )
            logger.debug(f"[BotEngine] Config type: {type(config)}")
            logger.debug(
                f"[BotEngine] Config fields: {config.__dict__ if hasattr(config, '__dict__') else 'N/A'}"
            )

            # Pass db session to engine for live mode API key access
            engine = BotEngine(bot_id, account_id, config, db=db)
            logger.debug("[BotEngine] Engine created successfully")
            ...
            logger.error(
                f"[BotEngine Error] BotConfig fields: {config.__dict__ if hasattr(config, '__dict__') else 'N/A'}"
            )
            logger.error(
                "[BotEngine Error] BotEngine.__init__ signature: (bot_id: int, account_id: int, config: BotConfig)"
            )
            logger.error(
                f"[BotEngine Error] Arguments passed: bot_id={bot_id} (type: {type(bot_id).__name__}), account_id={account_id} (type: {type(account_id).__name__}), config={config} (type: {type(config).__name__})"
            )
