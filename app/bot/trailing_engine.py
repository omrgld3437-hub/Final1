...
from app.bot.models import TrailingGridConfig, GridSlot, SlotState
...
class TrailingGridEngine:
    ...
    def _get_binance_client(self) -> Optional[BinanceClient]:
        ...
        if self.account.mode == "live":
            ...
            try:
                ...
                api_key = decrypt_account_api_key(
                    self.account_id, self.account.api_key_enc
                )
            ...
            try:
                if self.config.mode == "live" and self.binance_client:
                    order = self.binance_client.place_market_order(
                        self.config.symbol, "BUY", quote_order_qty=base_notional
                    )
                    # Parse fills
                    executed_qty = float(order.get("executedQty", 0))
                    float(order.get("cummulativeQuoteQty", 0))
                ...
                self.cycle_start_equity = self.quote_balance + (
                    self.base_balance * current_price
                )
                ...
                Ledger.record_trade(
                    ...
                    fee=total_fee,
                    slot_id=-1,
                    cycle_id=1,  # -1 for initial allocation
                )
            except Exception as e:
    ...
    def _init_grid_slots(self, p0: float):
        ...
        if self.config.up_grid_steps:
            ...
            steps = [
                self.config.up_grid_step_pct * (i + 1)
                for i in range(self.config.up_grid_count)
            ]

        for i, step_pct in enumerate(steps):
            target_price = p0 * (1 + step_pct / 100.0)
            slot = GridSlot(slot_id=i, direction="UP", target_price=target_price)
        ...
        if self.config.down_grid_steps:
            ...
            steps = [
                self.config.down_grid_step_pct * (i + 1)
                for i in range(self.config.down_grid_count)
            ]

        for i, step_pct in enumerate(steps):
            target_price = p0 * (1 - step_pct / 100.0)
            slot = GridSlot(slot_id=i, direction="DOWN", target_price=target_price)
        ...
        try:
            ...
            for slot in self.up_slots:
                if slot.state == SlotState.WAIT:
                    ...
                    slot.peak_price = max(
                        slot.peak_price or current_price, current_price
                    )
                    slot.state = SlotState.TRAIL

                elif slot.state == SlotState.TRAIL:
                    slot.peak_price = max(slot.peak_price, current_price)
                    drop_threshold = slot.peak_price * (
                        1 - self.config.up_sell_trailing_pct / 100.0
                    )
            ...
            for slot in self.down_slots:
                if slot.state == SlotState.WAIT:
                    ...
                    slot.trough_price = min(
                        slot.trough_price or current_price, current_price
                    )
                    slot.state = SlotState.TRAIL

                elif slot.state == SlotState.TRAIL:
                    slot.trough_price = min(slot.trough_price, current_price)
                    rise_threshold = slot.trough_price * (
                        1 + self.config.down_buy_trailing_pct / 100.0
                    )
                    ...
                    if current_price >= rise_threshold:
                        ...
                        if (
                            notional >= self.config.min_notional_quote
                            and self.quote_balance >= notional
                        ):
    ...
    def _process_profit_triggers(self, current_price: float):
        ...
        if self.total_sold_base > 0 and self.avg_sell_price > 0:
            drop_threshold = self.avg_sell_price * (
                1 - self.config.rebuy_trigger_drop_from_avg_sell_pct / 100.0
            )
            ...
            if self.rebuy_state == ProfitState.IDLE:
                ...
                rise_threshold = self.rebuy_trough * (
                    1 + self.config.rebuy_trailing_up_pct / 100.0
                )
                ...
                if current_price >= rise_threshold:
                    ...
                    if self.config.rebuy_use_proceeds_mode == "ALL_UP_SELL_PROCEEDS":
                        ...
                        rebuy_notional = self.quote_balance * (
                            self.config.rebuy_use_proceeds_value / 100.0
                        )
        ...
        if self.total_bought_base > 0 and self.avg_buy_cost > 0:
            rise_threshold = self.avg_buy_cost * (
                1 + self.config.resell_trigger_rise_from_avg_buy_pct / 100.0
            )
            ...
            if self.resell_state == ProfitState.IDLE:
                ...
                drop_threshold = self.resell_peak * (
                    1 - self.config.resell_trailing_down_pct / 100.0
                )
                ...
                if current_price <= drop_threshold:
                    ...
                    if self.config.resell_sell_mode == "SELL_ALL_DOWN_BOUGHT_BASE":
                        ...
                        resell_qty = self.base_balance * (
                            self.config.resell_sell_value / 100.0
                        )
    ...
    def _check_cycle_completion(self, current_price: float):
        ...
        if (
            self.rebuy_state == ProfitState.DONE
            or self.resell_state == ProfitState.DONE
        ):
            cycle_complete = True
