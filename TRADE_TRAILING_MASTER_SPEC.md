...
- **V5 exact shelf library (DPLV5 — aktif):** `app/services/dynamic_param_score/v5/` — **192.780** exact ana raf; route key 7-parça; shelf ID `DPLV5_*`; generator **`DPLV5_FORMULA_2`** (grid = max(cost_floor+margin, ATR-blend, scenario) × structure/direction/risk; `grid_reasoning` zorunlu); normal path exact lookup.
...
- **Kline penceresi (7 gün):** `KLINES_LIMIT_5M=2016`, `15m=672`, `1h=168`, `4h=42`; Binance >1000 için `_fetch_klines_paginated`. Telemetry: `data_window`. **DataHub boşken:** `data_collector._enrich_market_bundle_from_klines` — fiyat son kline close; 24h hacim 1h kline türetimi (`price_source=klines_close`, `volume_source=klines_1h`); NO_DATA/LOW_LIQUIDITY yanlış rejim engeli.
