...
- **Dynamic Param V6 (tek aktif motor):** `app/services/dynamic_param_score/v6/` · tam program: `docs/DYNAMIC_PARAM_V6_TECHNICAL_PROGRAM.md` · kaynak üretici: `tools/dynamic_param_v6/generate_v6_sources.py` (8 rejim · 63 alt · 231 mikro · **765 terminal**) · veri: `data/dynamic_param_v6/` (`scenario_tree_v6.json`, `behavior_catalog_v6.json`, `parameter_rulebook_v6.json`, `dplv6_profile_catalog.json`). **2.295** katalog profili (765 terminal × DEF/STD/ACT; factory doğrulama OK). Canlı motor: **`v6_scenario_classifier.py`** (öncelik: crash R8 → downtrend R7 → **trend/breakout R1/R5** → recovery R6 → **sıkı R4 kapısı** `range_score≥45` + `range_stability≥0.30` + güçlü trend yok → düşük likidite restricted) → katalog profili → ayarlayıcılar → `v6_quantizer` → **`v6_exchange_validator`** → **`v6_regime_behavior_spec.py`** (8 ana rejim × DEF/STD/ACT şartname + **R4 alt profilleri** `STD_LIQUID`/`DEF_OVERHEATED`/`DEF_LOW_LIQUIDITY`/`ACT_LOWER_BAND_BOUNCE`/`RESTRICTED_UNSTABLE`/`FRAGILE_BUT_LIQUID` + **R8** `DEF_PANIC`/`RECOVERY_RESTRICTED`; isimli sell dağılımları; trailing T0–T8 lattice zorunlu; `reason_codes`; likit coin F2 cezası hafif; R1/R5 base floor) → **`v6_opportunity.py`** (operational repair + validity) → **`v6_botparams_adapter.py`** (`BotParams` + PA/DM). **V6 apply_policy (params varken `safe_wait` yok):** `deployable` · `controlled_deploy` · `high_risk_controlled` · `technical_block` — geçerli piyasa senaryosunda `params=None` / “Parametre önerisi üretilemedi” yasak (yalnızca katalog/şema/teknik blok). `final_action` her zaman `CONTROLLED_GRID` (params geçerliyken `WAIT`+boş params yok). **PA telemetry:** `v6_pa_telemetry.py` — `indicators`, `sub_scores` (fee_efficiency strip + `*_risk_score` adları), `workability_score_1w`, `scenario_alignment`, `param_pool`, `data_window` PA/DM detay paneline beslenir. **PA/DM display:** `v6_pa_display.py` — profil etiketi `V6 Profil Kimliği`; fee modu `Canlı fee kullanılmaz` + sabit cost floor; güvenlik dili `Referans / bekle` yok; R8 alış kapalı metni “iki yönlü grid” değil; kâr döngüsü: `rebuy_enabled=false` → `profit_sell_enabled=false`; post-sell buyback `buy_grid_count=0` iken rebuy açık kalır. **Fee:** canlı fee yok; `DEFAULT_COST_FLOOR_PCT=1.2`. **Varsayılan:** `DPS_ENGINE_VERSION=v6`. Factory: `python3 -m app.services.dynamic_param_score.v6.v6_profile_factory`. Test: `tests/dynamic_param_score/test_v6_core.py`, `tests/dynamic_param_score/test_v6_adjusters_regression.py`, `tests/dynamic_param_score/test_v6_display_profit_loop_consistency.py`, `tests/dynamic_param_score/test_v6_opportunity_oriented_logic.py`, `tests/dynamic_param_score/test_v6_regime_behavior_spec.py`, `tests/dynamic_param_score/test_v6_regime_classification_gates.py`. Staging: `python3 tools/dynamic_param_v6/staging_v6_validation.py`. Simülasyon: `tools/dynamic_param_v6/simulate_all_v6_profiles.py` · `simulate_live_50_symbols_v6.py` (`WARN_OVER_PROTECTED_R6_MAJOR`, `WARN_R2_V1_LOW_ACTIVITY`, `ERROR_SAFE_WAIT_NULL_PARAMS`, vb.). Karşılaştırma arşivi: `reports/dynamic_param_v6/v5_v6_comparison_report.md`.
...
- **Target Allocation Layer:** `app/services/dynamic_param_score/allocation.py` — renderer çıktısından hedef base/quote oranı; mevcut portföyle `allocation_drift`; emir üretmez. Telemetry: `target_allocation`.
- **Order Intent Planner:** `app/services/dynamic_param_score/order_intent.py` — `BotParams` + `rebalance_plan` → grid/rebalance **niyet listesi** (`GRID_BUY`, `GRID_SELL`, `REBALANCE_BUY`, `REBALANCE_SELL`); min-notional ön filtresi; henüz borsaya gönderim yok. Telemetry: `order_intent_plan`.
- **Katman durumu (23 katman):** `data_collector` + `indicators` + `scoring` = Market Feature; `regime` + `scoring.risk` = Regime & Risk; tier detection = `param_pool/selector` SelectionContext; pool = Selector + Renderer; allocation/rebalance/order_intent = yeni katmanlar; feasibility + safety = mevcut; execution gate = `execution.py` (kısmi); lifecycle = `intent_ledger` + cancel flags (kısmi); explain/persistence = Audit (kısmi); learning = `persistence` + offline havuz (kısmi). **Eksik / sıradaki:** Template Stability, Cooldown/Anti-Overtrade, Emergency Protection (ayrı modül), Drift Monitor (canlı), Configuration Integrity gate, Order Intent → Execution bağlantısı.
...
- **Dynamic Mode V4 (tur başı):** Param Assistant ile **aynı** `DynamicParamScoreEngine.calculate_decision` zinciri (`run_source=dynamic_round_start`). Her tur başında route/profil/grid/dağılım/exposure yeniden hesaplanır; **tur başladıktan sonra tur ortası müdahale yok** (yeni seçim yalnız yeni `cycle_id` ile). `cycle_gate` hold varsayılan **kapalı** (`DYN_CYCLE_HOLD_ENABLED=false`). **Base/quote rebalance:** `rebalance_delta_total_pp = |Δbase|+|Δquote|`; **≤15pp → SKIP** (`SMALL_BASE_QUOTE_DELTA`, diğer parametreler uygulanabilir); **>15pp → tek seferlik** `REBALANCE_ONESHOT_BUY`/`SELL` (`MARKETABLE_LIMIT`, ladder yok); safety fail → `REBALANCE_DEFERRED`; cooldown `rebalance_cooldown_turns=2`. **Tur başlatma:** `deployable_grid` / `first_start_buy_only` dışında emir açılmaz; `START_BLOCKED_RETRY_PENDING` watchlist + backoff retry (`app/botengine/dynamic/start_retry_policy.py`). **Churn:** küçük spacing/trailing/dağılım farkında mevcut emirler korunur (`churn_policy.py`). Hard invariant: 1-grid deployable yok; 2-grid 50/50 yasak; 3-grid defensive eşit dağılım yasak.
- **Karar bağımsızlığı:** Her tur yeni `decision_id`; önceki turun seçili parametreleri yeni skorlamaya girmiyor; portföy/exposure canlı durumu girer.
...
- **Dinamik mod:** `cycle_manager.build_snapshot` DPS sonucunu `dynamic_snapshot.dps` + `applied` overlay olarak yazar; `smooth_against_prev` kaldırıldı.
