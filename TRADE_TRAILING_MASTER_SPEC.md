...
- **Tek merkezî motor:** `app/services/dynamic_param_score/` — hem Parametre Asistanı hem Dinamik Mod tur başlangıcı aynı `DynamicParamScoreEngine.calculate_decision()` çağrısını kullanır.
...
- **API:** `POST /api/dynamic-param-score/calculate` · `POST /api/param-assistant/calculate` (adapter, schema **`3.4`** — `selection_telemetry` + selection trace counts, `fee_display`, V4 `route_key`, `selection_path`, capacity/cost resolution; **`effective_risk_state`** + `display_regime_label` route rafından). **İstek (Param Assistant):** `symbol`, `budget`, `first_start_buy_only`, opsiyonel `base_balance_usdt` · `quote_balance_usdt` · `base_alloc_frac` (portföy senaryosu), `dry_run` (varsayılan `true` — canlı emir yok).
...
- **Kline penceresi (7 gün):** `KLINES_LIMIT_5M=2016`, `15m=672`, `1h=168`, `4h=42`; Binance >1000 için `_fetch_klines_paginated`. Telemetry: `data_window`.
