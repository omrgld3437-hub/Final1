...
- Gerçek istemci IP: `app/core/client_ip.py` → `client_ip_from_request`.
- `X-Real-IP` / `X-Forwarded-For` **yalnızca** peer `TRUSTED_PROXY_IPS` (varsayılan loopback) ise okunur.
- `X-Forwarded-For` eklemeli olduğu için **son** eleman kullanılır (ilk değil).
- Auth / rate-limit / audit / metrics aynı yardımcıyı kullanır (`app/api/auth.get_client_ip`).

## HATALAR.log (kök hata günlüğü)

- Dosya: proje kökünde `HATALAR.log` (gitignore’da; AI ve operatör okuyabilir).
- Yalnızca ERROR+; 90 gün saklama; aynı hata kısa pencerede tek satır + tekrar sayacı (`app/observability/hata_log.py`).
- Web (`app.main`) ve worker (`worker_main`) kurulumda `install()`; `persist_error` aynalar.

## Dynamic Mode V2 kill switch / churn

- Flag: bot `dynamic_mode_v2` (varsayılan false). Açıkken V1 dynamic hook atlanır.
- Kill switch: sağlık bozulunca düşer; sağlık dönünce `micro_risk_check` temizler. `RUNTIME_EXCEPTION` 900 sn cooldown sonra kendiliğinden açılır (`runtime.kill_switch_active`).
- Churn (saatlik limit + deadband): `projector.project(..., current=previous_applied_candidate)`; referans `_dynamic_v2_last_applied_candidate`. Acil kapatma: `DYNAMIC_V2_CHURN_CONTROLS=0`.
...
- **Reconcile NOT_FOUND → CANCELED:** Only when Binance **definitively** reports absence (`-2013` or clean empty history). Temporary network/429/inconclusive lookup errors leave the intent non-final (do **not** mark CANCELED). Log: `RECONCILE_NOT_FOUND account_id=... intent_id=... => CANCELED (stop re-query)`.
...
## Kullanıcı İşlem Geçmişi (User-Readable Activity Log)

Teknik `logs/app.log` ve DB `audit_events` / `error_logs` kayıtlarından **ayrı** sade Türkçe kullanıcı hareket günlüğü.
...
| Servis | `app/services/user_readable_activity_logger.py` → `UserReadableActivityLogger.write(...)` |
...
### Bot UI — cüzdan adet / trailing / rejim (2026-08)

| Kural | Değer |
|-------|--------|
| Cüzdan adet ondalığı | Coin’e göre dinamik (USDT/USDC: 2–4; küçük coinler 5–8); sabit 6 hane yok |
| Trailing vs grid | `trail < min(en yakın grid tetik %)`; eşit/büyük UI’da reddedilir (`TRAILING_TRIGGER_LIMIT` ile uyumlu) |
| Özet Ana rejim | Önce `pa_plan.headline` / `net_profile.headline`; bare `R3` → “Piyasa kararsız” yalnız kod fallback |
| Aylık kazanç | Bot bu takvim ayında açıldıysa `total − initial_capital` (toplam ile aynı baz); aksi halde ayın ilk snapshot’ı |
...
**Kurallar:** Stack trace, exception, raw JSON, API secret/token, açık IP kullanıcı loguna yazılmaz. Audit `log_event` eşlemeli olaylar (`LOGIN_SUCCESS`, `BOT_START`, `CHAT_*` vb.) otomatik aynalanır.
...
| GET | /api/dashboard/stream | Yes | Yes | Yes (account) | SSE snapshot (`DASHBOARD_SSE_ENABLED`, interval `DASHBOARD_SSE_INTERVAL_SEC`); UI EventSource + poll fallback |
...
| PATCH /accounts/{id}/settings | `password` / `new_password` reddedilir (400 PASSWORD_USE_AUTH_ENDPOINT); şifre yalnızca POST /api/auth/change-password (bcrypt) |
...
**Bağlantı düzelince otomatik devam:** Probe OK → `on_connectivity_restored`: `paused_error` botlar `running` + START; `running` botlar state `_pending_connectivity_stable` + START `connectivity_resume` (çift Bismillah yok). Yeşil `CONNECTIVITY_STABLE` (“Bağlantı kuruldu ve stabil · Bot sorunsuz çalışıyor”, Tür **Bağlantı**, `log-row-success`) **kurtarma olayı başına en fazla bir kez** (45 sn cooldown — worker restart + outage + orchestrator aynı olayı tekrar yazmaz). Akış: `emit_loop_auto_restart` yalnızca `mark_pending` (`WORKER_RESTART`); stabil satır `flush_pending_connectivity_stable` — outage recovery veya orchestrator tick sonu. `queue_and_flush_connectivity_stable` cooldown içinde atlanır. Worker cold boot: restart logu 60s throttle bypass (`last_emit &lt; worker.started_at`). UI: kurtarma logları event `id` başına ayrı satır. Tetik: worker `run_connectivity_auto_resume_pass`, bot sayfası `/health` + `/events`. **Bot sayfası sunucu erişim kaybı:** `SERVER_UNREACHABLE` sentetik kritik + kırmızı çerçeve; dönüşte tam log yenileme. **Resetle** / `POST /health/ack`: `CONNECTIVITY_STABLE` dismiss (`engine_log_ack`).

Tur sync / yeni fill → dosyaya ekle; bot fill `record_bot_trade_fill` ile anında (`quote_qty` = Binance `cummulativeQuoteQty`, `commission` = `fee_raw`). Alım/satım **Toplam** = USDT notional (`quote_qty`); fiyat = notional/miktar (UI ≥1000 USD fiyatlarda binlik virgül yok, örn. `$2038.77`). API `commission_usdt` döner; alım grid komisyonu base coin ise UI ham miktar + USDT karşılığı gösterir. **Fill dedup:** `.enc` içinde `seen_trades` — aynı Binance `trade_id` ikinci kez birleştirilmez (`sync_from_db_if_stale` tekrarı çift sayım yapmaz). İlk okuma `ensure_buysell_dedup_v1` bozuk alım/satım satırlarını `TradeNormalized`'dan yeniden kurar (`{id}.rev` → `buysell_dedup_v`). Binance sync bot etiketi: `order_intents.binance_order_id` veya `trades.order_id` (sembol+running tahmini yok). **Bot silme convert (`convert_base_to_quote`):** piyasa satışı `record_bot_close_convert_fill` ile anında **Bot + Ayserose** yazılır; sonraki Binance sync aynı `trade_id`/`order_id` ile Spot’a düşürmez. **Kaynak/platform etiketleri:** bot → `source_label=Bot[ · ad]`, `platform=Ayserose`; Ayserose UI manuel spot → `Ayserose`; yalnızca Binance `myTrades` sync → `Binance` (eski “Spot” etiketi kaldırıldı). Ledger `C_PLAT` (`A`/`B`). **İlk açılış / API anahtarı kaydı:** `bootstrap_tx_history_from_binance` — Binance `myTrades` → `TradeNormalized` → `rebuild_from_db` (365 gün) → `.enc`; `{id}.rev` içinde `bootstrapped: true`. `GET .../transaction-history` bootstrap bitmeden dosyadan filtreler; `sync=1` zorla yeniler. `GET .../transaction-history/revision` ~3.5s poll; değişince panel sessiz yenilenir.
...
### Dynamic Param Score Engine (canlı karar motoru)

- **Tek merkezî motor (canlı = V6):** `app/services/dynamic_param_score/` — Parametre Asistanı ve Dinamik Mod tur başlangıcı aynı `DynamicParamScoreEngine.calculate_decision()` → `calculate_decision_v6()` zincirini kullanır. **Canlı seçim zinciri (tek gerçek kaynak):** `MarketData` → `compute_indicators` (`roc_5m` = son 5m bar getirisi `n=1`; `return_1h_pct` = 12×5m ≈1h `n=12`; `pump_score`/`dump_score`/`fake_breakout_score`/`fake_bounce_score` 0–100 veya `None`) → `build_v6_input_contract` → `classify_scenario` (R1–R8 + `sub_profile_hint`) → **`apply_regime_stickiness`** (yumuşak aile flip’leri: R1/R3/R5 için aday **30 dk** kalıcı olmalı; uptrend→R2/R4 **60 dk**; R7/R8/hard-block anında) → `select_profile_key` → **35** `net_profile_library` profili → `PROFILE_COPY` **canonical headline** → `v6_pa_display` (başlığı yeniden yazamaz; yalnızca biçim/`market_status_plain`/why). 5m kapıları yumuşatıldı (`roc_5m` yalnız başına yetmez; pullback/overheat için daha net kısa vadeli teyit). R8 öncelik: **hard-block** > kapitülasyon/recovery altı > crash+likidite (`R8_LOW_LIQUIDITY_RESTRICTED`) > diğer. Telemetry: `regime_stickiness` + `classification_trace` (`selected_profile_key`, `canonical_headline`, `hard_block_reason`, skorlar, missing fields). **Tüketici bağımsızlığı:** `consumer_policy.py`. API: `POST /api/param-assistant/calculate` · `POST /api/dynamic-param-score/calculate`.
- **V4 (LEGACY / not used in live path):** Aşağıdaki V4 route/pool/shelf anlatımı tarihsel referans ve offline tooling içindir. Canlı `calculate_decision` V4 `param_pool.selector` / `route_key` / shelf lookup **çağırmaz**. Offline üretim ve audit araçları `param_pool/` + `param_generator/` altında kalır.
- **V4 legacy route sınıflandırma (`live_route_classifier_v4.py`, canlı seçimde kullanılmaz):** `market_signature_v4_from_live` — **lower_lows + ret24≤−6% + dd7≥20%** → `R7`; oversold → `R12`; vol eşikleri V4/V5. Selector ATR **`atr14_pct_1h`**. `apply_live_route_constraints` R7/R12+S2 base≤**30%**; 2-grid %50/%50 yasak.
- **Param Template Pool (LEGACY / offline):** `app/services/dynamic_param_score/param_pool/` — versiyonlu şablon havuzu (`v1.0.0` **50.000** · `v2.0.0` **100.000** · `v3.0.0` **200.000** · **`v4.0.0` **300.000** aktif template**, JSONL + SQLite + manifest + SHA256 + `selection_index.json`). Modüller: `models`, `generator`, `precision_generator`, `registry`, `selector`, `renderer`, `validators`, `sqlite_store`, `manifest`, `defaults`, `versioning`, `diagnostics`; V4 üretim: `param_generator/feature_bins_v4.py`, `scenario_specs_v4.py`, `param_library_builder_v4.py`, `v4_scoring.py`, **`v4_resolvers.py`**, **`route_manifest_v4.py`** (kanonik **10.710** ana raf = 7×17×9×5×2; budget/fee route dışı). Build v4: `python tools/param_pool/build_dps_v4_pool.py` veya `python tools/param_pool/build_param_pool.py --version v4.0.0`. Kritik DEFENSIVE raflar: `python tools/param_pool/seed_critical_defensive_shelves_v4.py` (`--scope mandatory|extended`; `--gaps-only` yalnız boş extended raflar; **varsayılan clone mod** kaynak profilden yeni `DPLV4_*_910xxx` `template_key` INSERT — kaynak raf korunur; `--no-clone` legacy in-place convert; mandatory fail=exit 1; `--strict` → post-index `optional_route_empty_total>0` ise exit 1; `--export-gaps DIR`). R2 kaynak yasak. Extended manifest: `extended_coverage_v4.py`. Zorunlu kabul: `tests/dynamic_param_score/test_v4_mandatory_acceptance.py` + `audit_v4/acceptance_v4.py`. Runtime: **`clean route_key`** indeksli lookup (20–500 aday) → V4 çok faktörlü skor → **resolver katmanı**; `PARAM_POOL_VERSION=v4.0.0` veya `data/param_pool/v4/*.sqlite` mevcutsa v4 yüklenir. **V4 clean route (5 parça):** `ASSET|REGIME|STRUCTURE|VOLATILITY|RISK` (ör. `A1|R6|S2|V3|DEFENSIVE`); **budget/fee/spread/min-notional/grid/distribution/trailing route’a girmez** — raf içi profil varyasyonu + `CapacityResolver` · `CostResolver` · `DataQualityResolver` · `SafetyResolver` · `MinNotionalResolver` · `RuntimeMicroAdjuster` seçim sonrası uygulanır. **300k profil** = 10.710 rafların içindeki alt davranış varyasyonları (eşit dağıtım yok; kritik raflar ağırlıklı). **V4 raf sistemi:** asset (A1–A7) · regime (R1–R17; R18/R19 alias) · structure (S1–S9) · vol (V1–V5) · risk (`NORMAL`/`DEFENSIVE`); profil ID `DPLV4_{ASSET}_{REGIME}_{STRUCTURE}_{VOL}_{RISK}_{SEQ}`; diskteki legacy 7-parça route runtime’da `normalize_route_key()` ile 5 parçaya indirgenir. Canlı akış: market signature → clean `route_key` → exact raf → aday skor → resolver → exchange validation → `final_action`; yasak fallback: R8→R2/R1/R3 · R7→R2 · S2↔S3 · V5→V1/V2 · LOWER_LOWS↔HIGHER_HIGHS · CRASH→BALANCED; raf boşsa **`generate_runtime_safe_profile()`** (`RUNTIME_SAFE_*`, pool profili gibi gösterilmez). `buy_grid_ladder_pcts` / `sell_grid_ladder_pcts` null olamaz; dağılım 2-grid %35/%65 (def %30/%70), 3-grid %15/%30/%55; `structure_fit==0` seçilemez; fee_bad → grid genişletme + `ACTIVE_DEFENSIVE_GRID` (WAIT/NO_TRADE değil); düşük bütçe grid spacing daraltmaz, grid count azaltır. **Tek seçim API:** `DynamicParamScoreEngine.select_profile(symbol, budget, market_data)` — Dynamic Mode ve Param Assistant aynı motor. Frontend karar alanı: **`final_action`** (eski `decision`/`wait_reason` karar kaynağı değil). Her seçim `telemetry.param_pool` + `selection_path` + `reason` + `selection_trace` içinde loglanır.
- **V5 exact shelf library (DPLV5 — KALDIRILDI):** V6 staging doğrulaması sonrası runtime kaldırıldı. `app/services/dynamic_param_score/v5/` silindi. Rollback: git branch/commit. Varsayılan motor: `DPS_ENGINE_VERSION=v6`.
- **Dynamic Param V6 (tek aktif motor):** `app/services/dynamic_param_score/v6/` · canlı profil kütüphanesi: **`net_profile_library.py` (35 operatör profili + `PROFILE_COPY` canonical headline)** · **sabit 4+4 grid / ~7 günlük tur / %10 odaklı sözleşme:** her profilde 4 alış + 4 satış; her seviye miktarı ≥%10 ve yön toplamı %100; normal profillerde en uzak grid ≤%10; yalnız yüksek vol / düşük likidite / pump / dengesiz düşüş / crash profillerinde %12–%15; Kapalı profiller 4+4’ü yalnız referans plan olarak tutar (`automatic_apply=false`, `reference_plan_only`); pipeline sonrası `seal_net_profile_shape` operatör ladder’larını `regime_behavior_spec`/opportunity ezmesinden korur · classifier: **`v6_scenario_classifier.py`** (öncelik: **parabolik pump R5** → **hard-block R8** → crash R8 altları → downtrend R7 → recovery breakout R6 → trend/breakout R1/R5 → sıkı R4 kapısı `range_score≥45` + `range_stability≥0.30` → R2/R3/fallback) → `select_profile_key` (`_HINT_MAP` + R8 hard-block > likidite) → ayarlayıcılar → `v6_quantizer` → `v6_exchange_validator` → `v6_regime_behavior_spec.py` → `v6_opportunity.py` → **`seal_net_profile_shape`** → `v6_botparams_adapter.py`. Diskteki 2.295 katalog / `data/dynamic_param_v6/` **rollback/offline tooling**; canlı PA/DM bu katalogdan seçim yapmaz. Move skorları: `move_scores.py` (eşik `SCORE_HIGH=70`, `SCORE_MEDIUM=40`). **PA/DM display:** `v6_pa_display.py` — `regime_headline` = `R{n} · PROFILE_COPY[selected_profile_key]`; semantic recovery / R8 likidite headline override **yok**; Kapalı referans ladder’ları display’de görünür, canlı BotParams buy yüzeyi kapalı kalır. **Fee:** canlı fee yok; `DEFAULT_COST_FLOOR_PCT=1.2`. **Varsayılan:** `DPS_ENGINE_VERSION=v6`. Test: `tests/dynamic_param_score/test_v6_selection_chain_integrity.py` (+ mevcut `test_v6_*`).
- **Rebalance Planner:** `app/services/dynamic_param_score/rebalance.py` — hedef `base_alloc_frac` ile mevcut exposure arasındaki drift hesaplanır; `deadband_frac` (varsayılan %5) altında `NO_REBALANCE`/`PASSIVE_REBALANCE`; buy tarafı `max_rebalance_per_round_frac` (varsayılan %8) ile kademeli limit emir planı; fee/risk/headroom/regime kötüyse alış rebalance yapılmaz. Template `params.rebalance_policy` ile modlanır; WAIT/NO_TRADE/SELL_MANAGEMENT template'lerinde buy rebalance kapalı.
- **Target Allocation / Order Intent (LEGACY yüzey; allocation.py / order_intent.py canlı V6 zincirinde zorunlu değil):** Offline/audit ve kısmi telemetry için kalabilir; canlı profil seçimini yönetmez.
- **Katman durumu (canlı V6):** `data_collector` + `indicators`/`move_scores` + `v6_indicator_adapter` + `v6_scenario_classifier` + `net_profile_library` + adjusters + opportunity + botparams + `v6_pa_display`. V4 pool/selector canlı katmanda değildir.
- **Profil aileleri (19) + v2 alt aileler:** `NO_TRADE` · `WAIT` · `SELL_MANAGEMENT_ONLY` · `RECOVERY_SELL` · `ULTRA_DEFENSIVE_GRID` · `DEFENSIVE_GRID` · `CAUTIOUS_BALANCED_GRID` · `BALANCED_GRID` · `LOW_FEE_WIDE_GRID` · `ACTIVE_RANGE_GRID` · `HIGH_CONFIDENCE_ACTIVE_GRID` · `TREND_TRAILING` · `BREAKOUT_PROTECTION` · `HIGH_VOL_PROTECTION` · `LOW_LIQUIDITY_WAIT` · `INITIAL_ENTRY` · `SMALL_BUDGET_SAFE` · `MICRO_BUDGET_WAIT` · `OVEREXPOSED_REDUCTION`. v2 `profile_subfamily` örnekleri: `PRECISION_INITIAL_ENTRY_PROFILE` · `GRADUAL_REBALANCE_BUY_PROFILE` · `FEE_WEAK_WIDE_GRID_PROFILE` · `BTC_PRESSURE_DEFENSIVE_PROFILE` · `ORDERBOOK_TIGHT_WAIT_PROFILE` (metadata `profile_family` altında).
- **Düşük fee ara profilleri (60–69 + BALANCED_RANGE):** `BALANCED_RANGE_60_69_FEE_BAD_WAIT` (base yok, bilinçli bekle) · `BALANCED_RANGE_60_69_FEE_BAD_SELL_MANAGEMENT` (base var, alış kapalı) · `BALANCED_RANGE_60_69_FEE_WEAK_WIDE_GRID` (`LOW_FEE_WIDE_GRID_PROFILE`, geniş spacing ≥ `4×total_friction_pct`). Sabit override şablonları: `DUMP_RISK_ANY_NO_TRADE` · `OVEREXPOSED_ANY_RECOVERY_SELL` · `TRENDING_DOWN_50_69_DEFENSIVE_WAIT_OR_SELL` · `RANGE_HIGH_VOL_70_89_ACTIVE_GOOD_FEE`. ACTIVE_GRID `FEE_BAD`/`FEE_WEAK` ile seçilmez.
- **Explain tutarlılığı:** `explain`, `risk_state`, alt skorlar ve `decision_id` aynı `calculate_decision` çıktısından gelir; `WAIT` kararı `params is None` NO_TRADE metnine düşmez; `NORMAL` risk iken explain’de savunmacı drawdown dili kullanılmaz.
- **Skor bucket (10):** `BLOCKED` 0–9 · `EXTREME_RISK` 10–19 · `VERY_DEFENSIVE` 20–29 · `DEFENSIVE_LOW` 30–39 · `DEFENSIVE_HIGH` 40–49 · `BALANCED_LOW` 50–59 · `BALANCED_HIGH` 60–69 · `ACTIVE_LOW` 70–79 · `ACTIVE_HIGH` 80–89 · `HIGH_CONFIDENCE` 90–100.
- **Rejim etiketleri (TR):** `LOW_LIQUIDITY` düşük likidite · `SPREAD_UNSAFE` güvensiz spread (explain/regime sözlüğünde Türkçe).
- **API:** `POST /api/dynamic-param-score/calculate` · `POST /api/param-assistant/calculate` (adapter, schema **`3.4`** — `selection_telemetry` + selection trace counts, `fee_display`, V4 `route_key`, `selection_path`, capacity/cost resolution; **`effective_risk_state`** + `display_regime_label` route rafından). **İstek (Param Assistant):** `symbol`, `budget`, opsiyonel `first_start_buy_only` (omit/`null` → **base yoksa otomatik `true`**, sadece alış grid; `false` → bilateral denemesi), opsiyonel `base_balance_usdt` · `quote_balance_usdt` · `base_alloc_frac` (portföy senaryosu), `dry_run` (varsayılan `true` — canlı emir yok). Tek kaynak: `data_collector.build_bot_context` + `resolve_first_start_flags`.
- **V4 canlı route tutarlılığı:** `market_signature_v4_from_live` — `volatility_percentile` (indikatör persentili, `volatility_score` değil) + HH/LL wide-chop; **R3+V4/V5 yasak** (reconcile → R4/R5); **BREAKOUT_RISK** → `R4` (pump exhaustion → `R15`); overbought+BTC baskısı → **R13/R14** + `risk_class=DEFENSIVE`; **karar zinciri sırası:** exact raf → **`exact_shelf_scored` (relaxed)** → fallback raf skor → pinned → **yalnızca** `production_pool_loaded` + `exact_route_candidate_count==0` + `fallback_candidate_count==0` iken `generate_runtime_safe_profile()` (`profile_source=runtime_synthetic`, `RUNTIME_SAFE_SYNTHETIC`); **`exact_route_candidate_count>0` iken runtime yasak** (`_selection_route_trace` flat telemetry). mandatory raflar seed sonrası `exact_route_candidate_count>0` beklenir (`seed_critical_defensive_shelves_v4.py --scope mandatory`, 24 kritik raf). **Final grid count sonrası** `distribution_policy.py` (`trim_side_distribution_for_context`: 2-grid 40/60·35/65·30/70·25/75; 3-grid 15/30/55·12/28/60·10/25/65·8/22/70; 50/50 yasak; `distribution_invalid`→`deployable=false`; deployable için buy≥2+sell≥2 veya `first_start_buy_only`; 1-grid→`single_probe_recommendation`; `result_type` ayrımı) · `resolve_adaptive_max_exposure` (V5/pump ≤%35) · `worst_case_exposure` hard cap — aşımda `deployable=false` / `WAIT_SAFETY` / `SELL_MANAGEMENT_ONLY` (`exposure_hard_cap_breach`); bilateral grid yeniden açma exposure breach sonrası yasak. UI rejim etiketi `route_key`/`regime_code` tek kaynak (R4→“Kırılım riski”).
- **Param Assistant öneri UI:** Havuzdan seçilen en uygun şablon **her zaman grid parametreleriyle gösterilir** (`result_type=recommended_grid`, `recommendation_config` / `ui_config`). Zayıf piyasa → savunmacı profil parametreleri; `deployable=false` olsa bile boş “güvenli bekle” kartı yerine veri sunulur. **`FEE_BAD_WAIT`** (ör. `BALANCED_RANGE_60_69_FEE_BAD_WAIT`): canlı karar `WAIT` kalır; UI’da referans olarak `BALANCED_RANGE_60_69_FEE_WEAK_WIDE_GRID` geniş grid parametreleri gösterilir (`reference_display_only=true`, uygulanmaz). Selector ikinci geçiş: `relaxed_pool_search` (tier/regime yumuşatma, deployable şablonlar). Yönetim kartı yalnızca gösterilebilir grid yoksa. **Özet kartı:** alış/satış gridleri, grid trailing, kar alım ve kar satış (tetik + trail) birbirinden bağımsız satırlarda; her grid tarafında miktar % toplamı **100** (largest-remainder). `recommended_grid` görünümünde kar alım/satış değerleri bilgi amaçlı gösterilir; canlı uygulama `rebuy_enabled` / `buy_disabled` kapılarına bağlıdır. **Forma uygula (`Kullanmak istiyorum`):** Base/Quote `ui_config.allocation_display.strategic_target` (DPS hedef exposure); `type=number` alanları (bütçe, dağılım %, grid sayısı) tek seferde atanır — ondalıklı animasyonlu yazım ara `31.` gibi geçersiz durumları bozar; çift taraflı grid için Base **25–65%** dışında forma **50/50** varsayılanı.
- **Yönetim kararı (WAIT/NO_TRADE):** Dinamik mod runtime’da riskli piyasa **kırmızı hata değil** bilinçli karardır. Adapter: `result_type=management_decision`, `management_mode` (`SAFE_WAIT`/`NO_TRADE`), `apply_policy` (`safe_wait`/`no_trade`), `can_apply_safe_overlay=true`, `safe_overlay` (buy kapalı, `cancel_existing_buy_orders`). **V6 istisna:** `pool_version=v6` ve `params` + `ui_config` varken `management_decision` dönülmez; `apply_policy` = `high_risk_controlled` / `controlled_deploy` ve savunmacı profil gösterilir. UI: `dmParamAssistantShowDpsManagementDecision` — yalnızca parametre üretilemediğinde; kırmızı yalnızca teknik hata (`dm-pa-error`).
- **Safe overlay:** `app/services/dynamic_param_score/safe_overlay.py` — `build_safe_wait_overlay` / `build_no_trade_overlay` / `build_data_stale_overlay`. Dinamik mod stale path manual buy grid geri yüklemez (`DATA_STALE_SAFE_WAIT`).
- **Runtime bayrakları (overlay + BotParams):** `buy_disabled`, `sell_only_mode`, `rebuy_enabled`, `resell_enabled`, `cancel_existing_buy_orders`, `cancel_existing_sell_orders`, `selected_template_key`, `pool_version`, `management_mode`. Execution gate: tüm BUY emirleri `BUY_DISABLED` / `SELL_ONLY_MODE` / `MAX_BUY_LEVELS_ZERO` / exposure / min-notional ile durdurulur.
- **Kline penceresi (7 gün karar + 10 gün trend):** `KLINES_LIMIT_5M=2016`, `15m=672`, **`1h=240`** (EMA200 için ≥200 bar; `DATA_WINDOW_1H_DAYS=10`), `4h=42`; Binance >1000 için `_fetch_klines_paginated`. Telemetry: `data_window`. **PA↔DM parity:** `live_parity.py` — Param Assistant `deployable` yalnızca Dynamic Mode tur başlangıcı da deploy edebiliyorsa `true`; `telemetry.live_parity_ok`, `pa_soft_deployable`, `dynamic_round_blocking`. **Senaryo uyumu (runtime):** `scenario_alignment.py` — V5 route → tek `regime_tag`; runtime `shelf_scenario_fit` + `structure_fit` + `grid_direction_fit` + `indicator_fit` + `applied_fit` → `combined_score`; `<70` → `SCENARIO_ALIGNMENT_LOW`, deploy kapalı; UI `scenario_alignment` (raf ideali vs uygulanan). **Fee skoru:** düşük 5m ATR majörlerde 1h ATR/√12 blend + likiditeye göre yumuşak payda (`scoring._fee_scoring_atr_pct`).
- **DEFENSIVE raf WAIT-only:** `sqlite_store.query_route_shelf_with_trace` — DEFENSIVE/CAUTION isteğinde yalnızca `WAIT`/`NO_TRADE` içeren raflar atlanır; deployable şablon bulunana kadar fallback devam eder; son çare aynı `A|R|S|V` ile `NORMAL` raf (`wait_only_escalation`, `defensive_fallback_overlay`). **Boş grid şablonları:** `hard_reject_v4` → `empty_deployable_grid` — `ACTIVE_DEFENSIVE_GRID` / bilateral aksiyonlarda `buy_grid_count+sell_grid_count==0` ve merdiven yoksa seçilmez (ör. `DPLV4_*_910007` WAIT-only raf artefaktı). Selector route shelf miss → legacy index (sıfır tarama yok). **Pinned generic fallback yasak (v4 yüklüyken):** `FALLBACK_*` anahtarları yerine kütüphane (`DPLV4_*`) veya sembol-özel `generate_runtime_safe_profile()`; `_library_deployable_last_resort` son tarama. **UI sembol izolasyonu:** `applyLastCreateParamsToForm` yalnızca aynı parite; cache anahtarı `accountId_symbol`; PA state sembol değişince sıfırlanır. **Bot create:** `config_source=param_assistant` + `param_assistant{…}` metadata `config_json`'da saklanır.
- **Selector tier filtreleri:** `liquidity_tier`, `volatility_tier`, `btc_risk_tier`, `order_reality_tier`, `max_total_friction_pct` hard filter; `requires_sellable_base` metadata + selector: `SELL_MANAGEMENT_ONLY` **satılabilir base yoksa asla seçilmez** (fresh budget dahil).
- **WAIT/NO_TRADE deployable:** pinned + havuz artifact `deployable=false`; validator zorunlu; on-disk düzeltme: `python tools/param_pool/repair_param_pool.py`.
- **Selector index (50k–300k):** `load_indexed_pool` memory index (`route_key_index` + legacy `dps_signature_index`; score bucket çoklu aralık + regime/risk/budget/fee/headroom/exposure ön daraltma) → hard filter (V4: `structure_fit==0`, null ladder, zıt senaryo fallback yasak; BTC_RISK_HIGH/ORDER_TIGHT → ACTIVE engel) → `selection_priority` + V4 weighted score; cache `_CACHED_INDEXED_POOLS`; p95 hedef `SELECTOR_P95_TARGET_MS=650` (50k) · `SELECTOR_P95_TARGET_MS_100K=700` · `SELECTOR_P95_TARGET_MS_200K=300` · **`SELECTOR_P95_TARGET_MS_300K=350`**.
- **Order intent execution:** `telemetry.intent_execution_enabled=false` — plan audit/telemetry; canlı emir kaynağı değil (henüz).
- **Eski ağır optimizer** (`app/services/param_optimizer/`) canlı akıştan çıkarıldı; yalnızca offline araştırma/kalibrasyon.
- **Param Assistant E2E audit (black-box):** Kullanıcı akışı `POST /api/param-assistant/calculate` üzerinden test edilir (iç motor çağrısı yok). Araç: `python3 tools/param_pool/param_assistant_e2e_audit.py` · rapor: `docs/PARAM_ASSISTANT_E2E_AUDIT.md` + `.json` · pytest: `tests/dynamic_param_score/test_param_assistant_blackbox.py`, `tests/e2e/test_param_assistant_user_flow.py`. Final kabul: 1-grid deployable=0, 2-grid 50/50=0, 3-grid eşit dağılım=0, worst>max+deployable=0, exact>0+runtime=0, runtime UI disclosure eksikliği=0.
- **Dynamic Mode (tur başı = Parametre Asistanı mutlak uygulama):** Param Assistant ile **aynı** `DynamicParamScoreEngine.calculate_decision` → V6 `net_profile_library` zinciri (`run_source=dynamic_round_start`). **Tur 1 dahil** her tur başında profil/grid/trailing/base-quote/kâr döngüsü yeniden hesaplanır ve `decision_to_overlay` ile **birebir** tura uygulanır (baseline × rejim çarpanı **yok**; `regime_multiplier` canlı yolda değil). **Tur başladıktan sonra tur ortası müdahale yok**. `cycle_gate` hold varsayılan **kapalı**. **Base/quote rebalance:** V6 `rebalance_plan` / `order_intent_plan` canlıda etkin (`intent_execution_enabled=true` tur başında); `rebalance_delta_total_pp = |Δbase|+|Δquote|`; **≤15pp → SKIP**; **>15pp → tek seferlik** oneshot; safety fail → `REBALANCE_DEFERRED`. **Uygulanamaz / R8 Kapalı / hard-block:** tur açılmaz (`round_pending`); buy kapalı; sabit **30 dk** rescan (`DYN_ROUND_START_RETRY_SEC=1800`, `NON_DEPLOYABLE_RETRY_MINUTES=30`). **Churn:** küçük farklarda mevcut emirler korunur (`churn_policy.py`).
- **Karar bağımsızlığı:** Her tur yeni `decision_id`; önceki turun seçili parametreleri yeni skorlamaya girmiyor; portföy/exposure canlı durumu girer. **İstisna (yumuşak rejim stickiness):** `apply_regime_stickiness` önceki R1/R3/R5 (ve soft sub-hint) çıktısını aday 30–60 dk kalıcı olana kadar tutabilir; R7/R8/hard-block bu hold’u kırar. Telemetry: `regime_stickiness.held` / `candidate_regime_id` / `confirm_remaining_sec` (+ PA `regime_stickiness_plain`). Store: `V6_REGIME_STICKY_STORE=memory|file|redis` (`v6_regime_stickiness_store.py`; file=`.run/v6_regime_stickiness.json` multi-worker same-host). Parametre/grid skorlaması yine bağımsızdır.
- **Hibrit canlı güvenlik (seal sonrası):** `apply_live_safety_overrides` — PA’da operatör 4+4 referans kalır; `R5_DEF_PARABOLIC_OVEREXTENDED` / `NEW_BUYS_PAUSED` için `live_buys_paused` + `reference_plan_only` + `automatic_apply=false` → canlı alış kapalı / `deployable=false`. Genel toparlanma heuristic (`ret24>0` & `dd7>5`) → **R6** + `R6_RECOVERY_ACT` (R5 breakout ailesine düşmez). R2 kapısı kademeli gevşetildi (RSI 42–58, `range_stability>0.45`). R7 `Açık — koşullu risk`; zayıf likidite+yüksek vol birlikteyse Kapalı’ya düşer.
- **NO_TRADE:** Gerçekten gösterilebilir grid yoksa `deployable=false`; Param Assistant yine de `pre_safety_params` üzerinden `recommended_grid` dönebilir.
- **Kalıcılık:** `dynamic_param_decisions` tablosu + `logs/dynamic_param_score/{decision_id}.json`. Disk retention: dizin **1 GiB** aşınca en eski loglar silinerek **100 MiB** hedefe indirilir (`log_retention.py`; env: `DPS_LOG_DIR_MAX_BYTES`, `DPS_LOG_DIR_TARGET_BYTES`). Dosya logları `filtered_out` (50k map) içermez — yalnızca `reject_examples` + özet.
- **Dinamik mod:** `cycle_manager.build_snapshot` DPS sonucunu `dynamic_snapshot.dps` + `pa_plan` + mutlak `applied` overlay olarak yazar; `smooth_against_prev` ve canlı çarpan overlay kaldırıldı.
...
- **Canlı fiyat / grid fiyat hücreleri:** `ui/assets/utils/coinPriceFormat.js` — sembol `tickSize` (`/api/spot/quick_data`) + fiyat büyüklüğü; ondalık **2–8** dinamik (`#botPriceEl`, grid tetik/tepe-dip). Tick varsayılanı (2 hane) düşük fiyatlı paritede `0,00` üretmez; majörde yanlış yüksek tick magnitude ile düzeltilir. Gösterim `tr-TR` ondalık.
...
- **Manager `wrn-engine`:** Hayalet uyarı yok — emilen tick / `BOT_START_SKIPPED` / geçici keys / `WORKER_FIRST_TICK_FAILED` → **INFO** veya `DEBUG`; `connectivity_resume` START döngüyü **iptal etmez** (`cancel_orchestrator_loop` yalnızca soğuk start). `state._ABSORBED_ENGINE_WARN_RE` + `logHumanize` geniş eşleşmeyi engeller. Gerçek arıza: bot engine log `HEALTH_CRITICAL` + `error_id`.
- **Bot süre (`stateHeroMetaDur`):** `state.bot_run_started_at` + `Bot.started_at`; `connectivity_resume` START sıfırlamaz; durdurulunca temizlenir; API `started_at` = `bot_run_started_at_iso` (eksikse engine log soğuk START heal).
- **`execution.py` tur kapanışı (2026-05-29):** Fonksiyon içi `from datetime import timedelta` (yalnızca `initial_allocation` dalında) Python’da `timedelta`’yı tüm fonksiyonda yerel yapar → sonraki `trail_profit_sell` kapanışında `UnboundLocalError` / `RUN_ACTION_EXCEPTION`; `record_trade` ve `CYCLE_END`/`CYCLE_START` atlanabilir. Düzeltme: modül import’u tek; fill `record_trade` tur kapanışından önce; arşiv defteri `GET .../trades` ile birleştirilir + eksik satırlar idempotent backfill.
...
- **Ek sağlık kodları (evaluate, throttled emit):** `PRICE_STALE_OR_MISSING`, `REPEATED_LOCK_BUSY`, `REPEATED_SLIPPAGE`, `CONNECTIVITY_DEGRADED`, `BOT_CONTINUES_ON_ERROR`, `LOOP_TASK_MISSING` (kurtarma metni). Eşikler (gürültü azaltma): tick stale warn ≥ `max(90s, interval×5)` · crit ≥ `max(300s, interval×12)` · fiyat bayat uyarısı ≥120s · sembol kilidi skip ≥18/15dk · recoverable tick hatası ≥180s süreklilik · yalnızca sarı uyarılarda Botlar sekmesi normal hızda sarı blink (hızlı blink yalnızca kritik+sarı karışımı).
- **Selector fallback sırası:** strict → `nearby_pool_search` (skor ±10, ≤2 yumuşak uyumsuzluk) → `relaxed_pool_search` (min skor 48, ≤4 yumuşak uyumsuzluk) → metadata fallback.
- **Hayalet önleme:** SKIP/ERROR ile aynı kod için HEALTH tekrarlanmaz; `FIRST_BUY_STUCK` son `initial_allocation` fill varsa atlanır; emit throttle (`_health_last_emit`, `_resilience_last_emit`). `BOT_LOOP_AUTO_RESTART`: aynı worker içinde 60s; **yeni worker boot** (`last_emit < worker.started_at`) her seferinde restart + stabil log.
...
| GET /api/bots-engine/{id}/session-alpha | GET | require_auth | resolve_session_alpha_performance (TTL 60s) | Oturum performansı hero; dakikada 1 poll |
| GET /api/bots-engine/{id}/perf-narrative | GET | require_auth | bot_perf_narrative (cycles + kural tabanlı yorum) | `?month=YYYY-MM` ay detayı; yoksa ay listesi |
...
| real_performance_pct | float \| null | session_alpha: balance_pct − coin_pct (run-start baseline) | Oturum performansı (alpha); **dönem filtresinden bağımsız**; baseline yoksa null |
| balance_change_pct | float \| null | session_alpha.balance_pct | Bakiye % (bot start → now); dönemden bağımsız |
| price_change_pct | float \| null | session_alpha.coin_pct | Coin/parite % (bot start → now); dönemden bağımsız |
| session_alpha_performance | object | resolve_session_alpha_performance | Oturum alpha; period=all/day/week/month aynı değer |
| bot_alpha_performance | object | session_alpha_performance alias | Geriye uyumluluk |
...
|-----------|-------|----------|
| pool_size | 10 | Sabit bağlantı sayısı (web/api SQLite ve PostgreSQL) |
| max_overflow | 20 | Ek bağlantı (web/api) |
| worker_sqlite_pool | NullPool | `DATABASE_ROLE=worker` + SQLite: her bot döngüsü kendi session'ını açar; QueuePool tükenmesini önler |
...
| SERVER_PUBLIC_IP_CACHE_TTL_SEC | 3600; startup + ipify/ifconfig keşfi (`app/services/server_public_ip.py`) |
| SERVER_PUBLIC_IP_REFRESH_SEC | 21600 (6h arka plan yenileme) |
| DASHBOARD_SSE_ENABLED | 1 (env); 0 → yalnız HTTP snapshot poll |
| DASHBOARD_SSE_INTERVAL_SEC | 3 |
...
| Adım | İşlem |
|------|--------|
| start_bot(bot_id, db) | Bot row; status='running'; `touch_bot_started_at` + state `bot_run_started_at` (connectivity_resume ise sıfırlanmaz); commit; soğuk start'ta seed_perf_chart; `_tasks`'da yoksa `create_task(_bot_loop)` |
...
| min_notional_guard | 10.0 | Min notional (USDT) |
...
**POST /api/bots-engine/{id}/delete (`convert_base_to_quote`):** Test/paper hesapta (`is_test_account`) base→quote dönüşümü atlanır; bot doğrudan silinir. Dönüşüm hatası worker-only / API anahtarı yok / test hesabı ise silme devam eder (400 yalnızca gerçek convert hatalarında). Canlı convert piyasa satışı `record_bot_close_convert_fill` ile işlem geçmişine **Bot** olarak yazılır (`platform=Ayserose`).
...
| 3b Trailing bant | Gerçekleşme tetik **üstünde** (alım, dip yok) veya **altında** (satış, tepe yok); fiyat tetik ile gerçekleşme arasında | Tetik **silinmez**; trailing devam (`CONTINUE_TRAIL`) |
| 4 Grid kaçtı | Tetikli satış: P &lt; tetik **ve** gerçekleşme ≥ tetik (tepe yeterli değil) | Tetik sıfırlanır (bekleme modu); işlem yok. Alım: P &gt; tetik iken yalnızca gerçekleşmeye ulaşınca işlem; tetik–gerçekleşme bandında sıfırlama yok (3b) |
...
| TX_HISTORY_BINANCE_STALE_SEC | 21600 | Ledger latest_time bu süreden eskiyse anasayfa GET Binance refresh tetikler | transaction_history_file_store.py |
| TX_HISTORY_BINANCE_REFRESH_MIN_SEC | 900 | Binance refresh minimum aralık (aynı hesap) | transaction_history_file_store.py |
...
**Live snapshot endpoint:** `GET /api/bots-engine/{bot_id}/live`. **Batch (dashboard):** `GET /api/bots-engine/batch/live?account_id=&bot_ids=1,2,3` (max 50; tek HTTP, sunucu 3s TTL önbellek + toplu `load_states_bulk`). Response: status, pnl_pct, equity, last_price, last_tick_at, last_error_code, initial_capital, daily_pnl_usd, daily_pnl_pct, daily_ref_usd. Optional flags: stale (last_tick_at > 30s), equity_unavailable. daily_pnl_* from state daily_ref_usd/daily_ref_date (TR gece 00:00 equity referans; gün değişince ref=equity; bot aynı gün açıldıysa ref=initial_capital). Source: in-memory snapshot from bot state only; no historical DB. TTL cache **3s**, key = bot_id; `save_state` → önbellek invalidate. UI poll: dashboard/bot detay **3s** (strip-price poll kaldırıldı; fiyat `/live`'dan). Response time < 15ms CPU (cache hit). 404 with structured error_code if bot not found. **Bots list:** `GET /api/bots-engine` — `load_states_list_meta` (N+1 `load_state` yok).
