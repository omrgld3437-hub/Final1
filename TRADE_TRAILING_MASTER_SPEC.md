...
- **Dynamic Param V6 (tek aktif motor):** `app/services/dynamic_param_score/v6/` · tam program: `docs/DYNAMIC_PARAM_V6_TECHNICAL_PROGRAM.md` · kaynak üretici: `tools/dynamic_param_v6/generate_v6_sources.py` (8 rejim · 63 alt · 231 mikro · **765 terminal**) · veri: `data/dynamic_param_v6/` (`scenario_tree_v6.json`, `behavior_catalog_v6.json`, `parameter_rulebook_v6.json`, `dplv6_profile_catalog.json`). **2.295** katalog profili (765 terminal × DEF/STD/ACT; factory doğrulama OK). Canlı motor: senaryo kimliği → katalog profili → ayarlayıcılar → `v6_quantizer` → **`v6_botparams_adapter.py`** (`BotParams` + PA/DM). **Fee:** canlı fee yok; `DEFAULT_COST_FLOOR_PCT=1.2`. **Varsayılan:** `DPS_ENGINE_VERSION=v6`. Factory: `python3 -m app.services.dynamic_param_score.v6.v6_profile_factory`. Test: `tests/dynamic_param_score/test_v6_core.py`, `tests/dynamic_param_score/test_v6_adjusters_regression.py`. Staging: `python3 tools/dynamic_param_v6/staging_v6_validation.py`. Karşılaştırma arşivi: `reports/dynamic_param_v6/v5_v6_comparison_report.md`.
