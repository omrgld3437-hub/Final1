...
> HEAD `21db228` · Toplam **158** commit · branch `main`
...
| Aktif branch | `main` |
| HEAD (kısa) | `21db228` |
| HEAD (tam) | `21db22880c7bef30401e6d4c6607108d391b60d2` |
| Remote durumu | `origin/main`'den **7** commit önde |
...
### 1. `21db228` — Bot health resilience, grid/profit UI polish, and engine hardening.
...
### 2. `2a5dcd3` — docs: sync GIT.md (git log)
...
### 3. `6066d76` — Grid trailing, Binance connectivity, and dashboard KPI hardening.
...
### 4. `06cef9a` — docs: sync GIT.md (git log)
...
### 5. `901f1a3` — Add grid trade modal, manager log UX, and bot detail sync fixes.
...
### 6. `9cd5850` — Update marketing submodule to latest landing and deploy changes.
...
### 7. `f2b5379` — Engine log UX, health alerts, grid trailing hardening, and tur trade fixes.
...
### 8. `a699226` — Checkpoint: pre profit-exit trigger fix (REST SSOT, UI, ops reorg).
...
### 9. `f203968` — Sunucu deploy.sh: guncel commit ciktida, degisti ise belirtilir
...
### 10. `f9fcf9b` — Linux HTML başlatma, bakiye doğrulama, dual PNL (Cash/Inventory), perfPanel+tradesPanel, Aktif Emirler sadece Anasayfa
...
### 11. `2fea40c` — Admin: askı cache invalidation + çevrimiçi last_seen 2dk; sohbet flicker/typing düzeltmesi; coin logo cache 31 gün
...
### 12. `da82dd3` — Sohbet: admin yazıyor göstergesi + kullanıcı çevrimiçi/çevrimdışı nokta (yeşil/kırmızı)
...
### 13. `8fa1d4a` — Sohbet: anında mesaj (admin poll 2.5s, kullanıcı poll 2.5s, gönderim sonrası yenileme, welcome 1.2s)
...
### 14. `4a93a08` — Login: kripto logo flicker onleme - liste bir kez render, sonra sadece fiyat/chg guncelle
...
### 15. `c82c2d7` — Aktif Emirler paneli: emir yokken her zaman İşlem Geçmişi altında (txPanel.after)
...
### 16. `37791ce` — Admin: sunucu paneli, audit, hata logları, API rehberi
...
### 17. `a411a4b` — fix: logo flicker, price blink, bot performance period update; suppress ProactorBasePipeTransport log
...
### 18. `535a648` — omeraltin.com kapanma: HTML watchdog + auto_start + start.py sertlestirme
...
### 19. `ae3d5d3` — Admin tabs: mobile Sekmeler fix + perf (store, preload, instant switch)
...
### 20. `801d68d` — Bot sayfası: Başlat kaldırıldı; 401'de Hata (API anahtarı) gösterimi; strip fiyat/24h/mini grafik layout kayması düzeltmesi
...
### 21. `b2717f2` — Manager: GET / redirect, 401/ConnectionResetError log bastırma; Oluştur=anında başlat; Başlat butonu sadece stopped; Leaderboard parametreleri görüntüle + modal
...
### 22. `ef90350` — Bot 401: pause bot on Binance Unauthorized, set API_UNAUTHORIZED and ERROR event for UI
...
### 23. `9a1b6f8` — UI/UX: flicker fixes, mobile padding, admin tabs, leaderboard, chat; deploy nginx config
...
### 24. `d0c1f26` — Deploy: Nginx domain eski gorunum notu (DEPLOY.md) + show-nginx-config.bat (Windows)
...
### 25. `89cf45b` — start.bat: teshis ciktisi - Dashboard (diskte) surum + build-info URL + eski gorunum notu
...
### 26. `e81ea4f` — build-info login fix + no-cache UI: path normalize, route ilk sırada, no-cache middleware, DEPLOY notları
...
### 27. `e8cd6f8` — api/debug/build-info: lockdown whitelist'e ekle (giriş olmadan erişilebilsin)
...
### 28. `61248cf` — Deploy: Değişiklikler yansımıyor rehberi + /api/debug/build-info endpoint
...
### 29. `f5fb145` — Bot engine, routes, cycle_ledger, dashboard snapshot, UI: tüm kalan değişiklikler
...
### 30. `a3b8688` — Leaderboard: sadece kârda botlar; Bulunamadı mesajı. Admin: sarı çerçeve, tab sarı+kayma, koyu tema, hover glow
...
### 31. `9153e39` — Admin: tab slider indicator + içerik kayma animasyonu
...
### 32. `cd4e83d` — Copy Trading + Global Leaderboard; admin hesap kartları stil
...
### 33. `4598ac0` — Spot modal: hemen açılma + yüzde flicker düzeltmesi; ilk giriş popup sırası + mobil X
...
### 34. `c170a91` — binance_spot: _binance_ip_ban global kaldırıldı – mutable state ile 'used prior to global declaration' hatası giderildi
...
### 35. `7a0817a` — Wallet: bot/emir kilitli strip+tablo; Trade favori: sembol/fiyat tam görünsün
...
### 36. `0698e05` — Dashboard, bot panel, popup, perf: İşlem geçmişi görünürlük, fiyat blink, live K/Z, popup etiket, SLOW_REQUEST optimizasyonu
...
### 37. `e210bf5` — Dashboard & API: perf, UX, işlem geçmişi, yenilemede sekme koruma
...
### 38. `64fb33e` — ui: dashboard trade view + wallet refresh + bot detail back link
...
### 39. `c1012b3` — fix: bot running but no real Binance trades — run_id, reconcile NOT_FOUND, verify-before-repair
...
### 40. `3dfdd54` — feat: BOT_TICK_SUMMARY + BOT_TEŞHIS_DUMP on loop start; favori coin layout fix
...
### 41. `7f879df` — fix: live/paper audit logs, Bot Performansı %, Favori Coinler, İşlem Geçmişi paneli, manager favicon/alerts
...
### 42. `218a086` — fix: avoid logout on 401 UNAUTHORIZED (missing_token); only SESSION_NOT_FOUND triggers session invalid; auth missing_token log at DEBUG
...
### 43. `ca855ba` — fix: force mainnet for non-test accounts; log BOT_ACCOUNT_CTX (paper_mode, testnet) at tick start
...
### 44. `1cdf887` — fix: sync virtual_wallet from state when initial_allocation_done so balance not overwritten; repair log
...
### 45. `113c237` — fix: real account never runs in paper mode (paper only when test user and no API keys)
...
### 46. `d40a24e` — fix: repair set initial_allocation_done to stop repeat repair; data_hub get_all_prices snapshot
...
### 47. `edb7864` — fix: initial allocation repair use intent client_order_id; dashboard bot performance range flicker
...
### 48. `7891d83` — feat: pre-fill bot create form with last used params (localStorage)
...
### 49. `84ec6d2` — fix: initial allocation 1s retry; v5 use strategy next_wake; remove Performans raporu block; spec
...
### 50. `af57288` — fix: restore Favori Coinler layout to previous working state (inline list style, simpler mobile CSS)
...
### 51. `c6a7d68` — fix: bot detail live price + initial allocation state UX
...
### 52. `d2741b4` — fix: bots_delete skip convert on worker-only error, proceed with delete
...
### 53. `1b0bb33` — fix: remove redundant local import of update_intent_filled to fix UnboundLocalError
...
### 54. `e3bb759` — fix: chart page mobile – flex layout, 44px touch targets, no horizontal overflow
...
### 55. `3b6989a` — fix: Bot Create modal mobile – viewport lock, body-only scroll, form spacing
...
### 56. `d2719ec` — perf & UI: finance/trades 365d cap; snapshot 5s; bot perf flicker fix; tx history order grouping + Binance platform + layout; mobile trade favs & Bot Create modal
...
### 57. `6c5dd19` — fix: spot order HTTPException re-raise (no 500); mobile trade fav layout 30/50px from logo
...
### 58. `2bfb377` — Trade favorileri: sembol logonun yanında, fiyat 30 boşluk, yüzde 50 boşluk hizalama
...
### 59. `7dc4458` — fix: Bot Performansı PnL flicker – sadece değer değişince DOM güncelle
...
### 60. `e8cb0a2` — Dashboard: Trade favorileri mobil sığdırma, sembol/logo hizalama, 30 boşluk spacer
...
### 61. `cf33970` — fix: bootstrap 404, mobile wallet, spot order from web, ticker desktop-only
...
### 62. `29335ec` — fix(mobile): wallet data on mobile — sync bootstrap wallet from dashboardStore to assetsState
...
### 63. `fd2b91d` — fix(mobile): hide top ticker strip; ensure Binance data loads on mobile
...
### 64. `eee7bea` — fix: snapshot cache-only wallet + dashboard bootstrap + mobile ticker hide
...
### 65. `51ba364` — mobile: root fix - tabs + prices visible
...
### 66. `e11dfec` — fix: SyntaxError in api_debug_wallet_diag snapshot_asset_count
...
### 67. `6ef26df` — wallet: CURSOR TASK - normalizeAndApplyWallet, debug overlay, trace logs, diag endpoint
...
### 68. `e6f75cc` — fix: Wallet loading / cüzdan hiç gelmiyor — 5 patch uygulandı
...
### 69. `43b775a` — fix: dashboard flicker, wallet loading, appbar mobile, manager Windows Server
...
### 70. `cb7fdc9` — fix: Flash Home fallback + mobile Trade favori/arama
...
### 71. `1b22c82` — fix(ui): preserve last good spot balance in Flash Home walletCachedToAssetsState
...
### 72. `caab28d` — fix: Binance wallet flash to 0 + mobile appbar center width
...
### 73. `3976ba6` — fix(ui): mobile dashboard appbar single-row layout and alignment
...
### 74. `51c93ef` — Binance: 429 reduce myTrades to 40 symbols + delay + cooldown 10min; 401/timeout: 8s timeout, timestamp fallback, wallet_error in snapshot
...
### 75. `048c42f` — start.bat: checkout stop.bat before pull to avoid merge abort; note for file-in-use error
...
### 76. `3718194` — Manager: global actions include HTML, restart(html), loading state and feedback for all buttons; stop.bat and DEPLOY notes
...
### 77. `08d71d1` — Add app.services.dashboard_snapshot to repo; 503 on snapshot when module missing
...
### 78. `25fa288` — fix(stop.bat): port-based kill first, locale-safe netstat, run-as-admin hint
...
### 79. `01a9542` — docs: add app.api.utils to ModuleNotFoundError section in DEPLOY.md
...
### 80. `4de2f98` — Add app.api.utils (fields) for snapshot; defensive import in routes (fallback if module missing)
...
### 81. `00e4eab` — Server: middleware optional (app.middleware yoksa baslar), middleware repoya, worker DataHub warmup, deploy CRASH_LOOP notu
...
### 82. `b65acc8` — deploy: ModuleNotFoundError (app.core / intent_ledger) icin git pull ve manuel kopya notu
...
### 83. `766165d` — Add app.core module (auth.token_utils, config, constants, errors, security) for server - fix ModuleNotFoundError: app.core
...
### 84. `bc08bf0` — Windows: uvloop/httptools atlama (ModuleNotFoundError on server)
...
### 85. `354b34d` — Add missing botengine modules (intent_ledger, kill_switch, reconcile, scheduler, bot_run, errors, user_stream) for server deploy
...
### 86. `d7a979c` — UI: popup 405 fix, Görüntüleyen+UTC+3, auth log, appbar sticky, login telefon label/placeholder, dm-tabs sola
...
### 87. `0a724da` — UI: çıkış butonu ortala/KPI hiza, popup detay+okuyanlar, Ayarlar sekmesi sağa
...
### 88. `1bcc836` — fix(auth): do not logout on boot_id change; validate via whoami
...
### 89. `5bda1f6` — fix: perf chart modal crosshair guard (pt + try/catch), cache bust v=2
...
### 90. `46e3874` — Login-forex paralel; rebalancing USDT satiri; finance trades 90s timeout + cache bust; perf_chart crosshair param guard
...
### 91. `af74323` — Login-forex: altin yedek Binance PAXGUSDT; Manager: web durumu port 8000 ile
...
### 92. `248d1a2` — Login: döviz/altin backend /api/login-forex proxy (CORS cozumu)
...
### 93. `8686cb2` — 503 hata mesaji: backend detail.message goster (BINANCE_MASTER_KEY uyarisi)
...
### 94. `41ff1ef` — Login: kripto fiyatlari backend /api/login-crypto uzerinden (CORS cozumu)
...
### 95. `5da991d` — Admin panel: giris yapan adminin kendi hesabi tile listesinde gosterilmesin
...
### 96. `9b8ee7e` — Login: ilk admin sentinel bcrypt kaldirildi (Invalid salt hatasi duzeltildi)
...
### 97. `b83fce6` — İlk admin otomatik, varsayılan şifre kaldırıldı; ilk girişte yazılan şifre kalıcı. Kullanıcı yoksa 'Kullanıcı bulunamadı' mesajı.
...
### 98. `7c6c910` — create_first_admin.py: ilk admin yoksa olusturur (Admin / Omeromer01.)
...
### 99. `1c595b3` — Login 401: localhostta debug_hint (user_not_found / invalid_password) ve arayuzde gosterim
...
### 100. `58e28a4` — Giris: sifre dogrulama NFC + NFD + raw strip ile tam uyumluluk
...
### 101. `40174e9` — Silinen dosyalar: SEO_MASTER_*, calistir.bat
...
### 102. `15efe04` — Admin varsayilan sifre Omeromer01., giris sonrasi yeni sifre zorunlu
...
### 103. `6b9b6c3` — guncelle.bat mesaji; deploy_windows.sh Windows sunucu deploy
...
### 104. `0b57ef0` — Admin sifresi giris yapmadan sifirlama: ADMIN_PASSWORD_RESET_SECRET ile endpoint
...
### 105. `584f60e` — Admin: telefon ile sifre atama endpointi (yayinda giris icin sifre senkron)
...
### 106. `efc00af` — Giris: sifre NFC/NFD normalizasyonu - local ve yayinda ayni sifre kabulu, invalid password log
...
### 107. `1b99ed6` — guncelle.bat: sunucuda start.bat eski kaldiginda once calistirilacak proje guncelleme scripti
...
### 108. `8bf6d31` — Giris: sifre dogrulama guclendirildi, trim ve hash str/bytes destegi
...
### 109. `b05f62e` — start.bat: batch echo duzeltmeleri, calistir kelimesi stir hatasini onlemek icin baslat ile degistirildi
...
### 110. `25e105e` — start.bat: git pull ciktisi goster; Omeraltinhtml yoksa yonlendirme mesaji
...
### 111. `044e331` — start.bat: guncel commit goster; Omeraltinhtml klasorunu adinda omeraltin gecen her varyanttan bul
...
### 112. `be4f246` — start/restart.bat: her calistirmada git pull; Omeraltinhtml klasor adi kontrolu
...
### 113. `a388351` — Login: şifre butonu yayında tutarlı, login.css v=2 cache-bust
...
### 114. `455adb7` — Şifre göster/gizle butonu: göz ikonu ve düzenli stil
...
### 116. `e529605` — init: project source
...
### 117. `fe8c9cf` — chore: prepare repo for deploy (ignore runtime/data)
...
### 119. `3e6fcbc` — chore: prepare project for git-based deploy (no logic changes)
...
### 120. `78acfea` — TRDCA bakiye düzeltmesi, login mobil scroll, favori coinler daraltma
...
### 121. `23d2d4e` — Kurulum: ilk calistirmada otomatik Kurulum.bat. start.bat: 8080/Omeraltinhtml terminal mesajlari. Kadran: tek renk koyu yesil + kucuk sik balik sirti efekti
...
### 122. `2a2cc3e` — Login: sözleşme onayları, modal scroll, Geri/Okudum anladım. stop/start/restart.bat düzeltmeleri, deploy ve command klasörleri
...
### 123. `c871840` — Admin: tema, mobil sekmeler, sunucu paneli, ticker gizleme, sekme hatırlama
...
### 124. `c2b240e` — Login: çıkış uyarısı kutu içinde kısa, Güvenli çıkış yapıldı, mobil Sekmeler dropdown sayfa içinde
...
### 125. `2618660` — Login: topbar bayrak sağda/buton boyu, kayan yıldız düz, mobil tab 50/50, modal sekmeler ortala
...
### 126. `e7b6395` — Login: saatler koyu Rolex yeşili ve zengin görünüm, nebula sağda sabit ve görünür
...
### 127. `b1dc19c` — Admin panel mobil: sekme dropdown hesapların üstünde görünsün (z-index düzeltmesi)
...
### 128. `86882a5` — feat: MULTI bot Sonlandır ve Sil - tüm coinleri quote'a çevir, modal metni güncelle
...
### 129. `e8ccbd7` — Start/stop sadece final1 kokunde: start.bat, stop.bat, restart.bat + .command; diger tum start/stop dosyalari kaldirildi
...
### 130. `e823e3c` — Manager panel: özet flicker düzeltmesi, WS errors_ring/warns_ring birleştirme, Hata/Uyarı panel etiketleri, 401 throttle özet satırı
...
### 131. `501052e` — Hata logları sıfırlama: backend clear endpoint + frontend entegrasyonu (yenileyince kalıcı temiz)
...
### 132. `2bebb02` — TRDCA Pro+ parametre ekranı, test hesabı iyileştirmeleri, hata logları ve UI düzeltmeleri
...
### 133. `041c4ff` — Test hesabı (test_local): sadece localhost giriş, paper mod, 10.000 USDT sanal bakiye
...
### 134. `3d4e9ee` — TRDCA Pro+ strateji entegrasyonu: backend (strategy_tick, apply_fills, DCA/TRB), orchestrator snapshot, execution batch, UI parametre ve detay sayfası
...
### 135. `2dff087` — Mobil uyumluluk, admin/error-logs API, Windows launcher ve genel güncellemeler
...
### 136. `4f745c1` — Manager panel (7999): hız, restart, log ve overview iyileştirmeleri
...
### 137. `dcb5cd9` — logs: .meta yeşil metin, #panelAnasistem dış çerçeve cırtlak pembe
...
### 138. `184d766` — Rebalancing entegrasyonu, UI/API düzeltmeleri, Binance saat senkronu
...
### 139. `0951fa5` — Log sunucusu düzeltmeleri: Python 3.9 uyumu (Optional[int]), Server Start'ta terminalde log, Yeniden Başlat sonrası poll+Sayfayı yenile, Anasistem sunucu istekleri
...
### 140. `907e638` — Worker sekmesi Toplam istek canlı güncelleme: worker_main sayacı, local_logs fallback, manager okuma sırası
...
### 141. `eb3c400` — Değişiklikler kaydedildi: Server Start, local_logs, orchestrator, worker_main, local_web_worker_helper, manager_backend, logs
...
### 142. `8fe922a` — Log sınıflandırma düzeltmesi: INFO/WARNING yanlış hata sayılması engellendi
...
### 143. `e9d5f2a` — Proje güncellemesi: Server Start/Stop, loglar sayfası (7999), web/worker ayrı yönetim, Sıfırla/401/renklendirme düzeltmeleri
...
### 144. `fb7a619` — feat: günlük PnL bot silindiğinde sıfırlanmasın — account_daily_realized_pnl cache
...
### 145. `fe407f5` — Clean cycle boundaries + compounding + perf chart + SELL balance guard
...
### 146. `f17a949` — Cycle PnL fix (fee-aware), perf chart, XRAY doc
...
### 147. `0d41fe9` — Grafik 1m/5m penceresi, canlı uç setData, tur/trades canlı güncelleme, işlem satırına gerçekleşme %, reference_price düzeltmesi
...
### 148. `206a309` — Güvenlik: breach shutdown + hesap askıya alma uyarısı; grid tamamlanınca tepe/dip/gerçekleşme dondurma; fill fiyatı 4 ondalık; fill fiyatı state ile grid UI
...
### 149. `b5a5e57` — Grafik: sunucuda state, yeni tarayıcıda yükleme, sekme kapalıyken kayıt
...
### 150. `287db18` — UI: Genel buton/perfGenelValue, işlemler yenileme, filtre-tablo boşlukları
...
### 151. `037f675` — UI: grafik yüzdeleri, grid/kar panelleri, cycle report, header/state sırası, bot tipi başlık, refD hatası
...
### 152. `aa21968` — Sonlandır/sil: base→quote seçilince Binance market satış; grafik bot bazlı clearForBot + başlatınca sıfırla
...
### 153. `6f13e74` — Paper→live zorunlu, initial allocation sıfırlama, ortalama maliyet execution_price, grafik bot bazlı, form varsayılanları
...
### 154. `73e411b` — Performans grafiği (Lightweight Charts), bot başlangıç/duration, cycle raporu, PNL/kar satışı, hata log API, grafik Sıfırla kaldırıldı
...
### 155. `ef44e08` — UI: cycle trades labels, symbol chart modal back fix, perf chart debug doc
...
### 156. `0e1e645` — Dashboard & bot: KPI bot bakiyesi, günlük PnL (tur kârı), işlemler sync, Toplam K/Z, performans grafiği
...
### 157. `6fa4eae` — Dashboard & bot: KPI günlük ref, işlemler TR saati, grid qty_pct, PNL kartı, blink, buton stilleri
...
### 158. `397ead6` — Performans raporu canlı veri, grid referans düzeltmesi, Bot Logları, bileşik tur
