...
if "%PY%"=="python" (
  ...
  call "%ROOT%\ops\Kurulum.bat" --icerde --otomatik
  if errorlevel 1 (
    echo.
...
REM --- marketing sitesi 8080 ---
echo [4/4] Marketing site 8080 baslatiliyor...
set "HTML_PID=%ROOT%.run\html.pid"
set "HTML_DIR="
if exist "%ROOT%marketing\start.py" set "HTML_DIR=%ROOT%marketing"
if not defined HTML_DIR if exist "%ROOT%marketing\calistir.bat" set "HTML_DIR=%ROOT%marketing"
...
if exist "!STARTPY!" (
  ...
  echo      marketing/ klasoru veya start.py bulunamadi, atlaniyor.
  echo      Sunucuda: git pull ve git submodule update --init --recursive
  echo      veya marketing/ klasorunu kopyalayin.
)
...
echo   HTML    : http://127.0.0.1:8080  - marketing site
